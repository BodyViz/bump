...
void valueMustBePositive(int value) {
    if (value < 0) {
        std::cout << "valueMustBePositive FAILED with a value of " << value
                  << std::endl;
        throw bump::InvalidArgumentError("The value cannot be negative",
                                         BUMP_LOCATION);
    }

    std::cout << "valueMustBePositive executed SUCCESSFULLY with a value of "
              << value << std::endl;
}
...
void removeItemFromVector(std::vector<double>& values, int itemIndex) {
    ...
    if (itemIndex < 0 || itemIndex > (int)values.size() - 1) {
        std::cout << "removeItemFromVector FAILED with an itemIndex of "
                  << itemIndex << std::endl;
        throw bump::OutOfRangeError(
...
void tryToTypeCast() {
    ...
    if (base3) {
        ...
        throw bump::TypeCastError("Failed to typecast base1 to a Derived",
                                  BUMP_LOCATION);
    }
    ...
    if (base3) {
        ...
        throw bump::TypeCastError("Failed to typecast base2 to a Derived",
                                  BUMP_LOCATION);
    }
}
...
void doubleThrow() {
    try {
        ...
        // You can use the extendDescription method to append an additional
        // description to the exception
 ...
 *
 * To view the bump::Exception hierarchy, please refer to the documentation
 * provided in the <bump/Exception.h> header.
 *
 * Throughout the Bump library, there are certain cases guarded by exceptions
 * when used incorrectly. These safeguard crashes and give you, the user, the
 * ability to determine what to do at runtime. This example breaks down how you
 * can handle these exceptions if you choose to. It also shows you how to use
 * multiple throws to continue sending the same exception up the stack with
...
int main() {
    ...
    // Demonstrates how to handle a bump::OutOfRangeError by catching it as a
    // bump::Exception
    try {
        std::cout << "\n=================== bump::OutOfRangeError Example "
                     "==================="
                  << std::endl;
        ...
        // This will only catch a bump::InvalidArgumentError exception. Since
        // this is not the type of exception throw in the removeItemFromVector
        // function, this will never actually get called unless we modify the
        // function to also throw this type of exception.
        std::cout << e.description() << std::endl;
    } catch (const bump::Exception& e) {
        // We can also catch exceptions by using base classes higher up in the
        // bump::Exception inheritance hierarchy. This block will in fact catch
        // any exception thrown by Bump since bump::Exception is the base class
        // for all actual exceptions.
        std::cout << e.description() << std::endl;
    }

    // Demonstrates how to handle a bump::TypeCastError.
    try {
        std::cout << "\n=================== bump::TypeCastError Example "
                     "==================="
                  << std::endl;
    ...
    // Demonstrates how to use multiple throws to append additional info to the
    // exception.
    try {
        std::cout << "\n=================== Multiple Throws Example "
                     "==================="
                  << std::endl;
