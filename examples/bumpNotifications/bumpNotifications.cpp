...
public:
    ...
    Renderer(const bump::String& name = "")
        : _name(name), _numRenderPasses(2), _redrawRequestCount(0) {
        // Create a key observer for the requestRedraw() method
        // This looks daunting, but is really quite easy. Just following this
        // example...
        //    - bump::Observer* name = new bump::KeyObserver<this_type>(this,
        //    &func_pointer, notification_name);
        bump::Observer* redraw = new bump::KeyObserver<Renderer>(
            this, &Renderer::requestRedraw, "RequestRedraw");
        ADD_OBSERVER(redraw);

        // Create object observers for the updateNumRenderPasses(), changeName()
        // and changeNameWithString() methods This looks daunting, but is really
        // quite easy. Just following this example...
        //    - bump::Observer* name = new bump::ObjectObserver<this_type,
        //    object_type>(this, &func_pointer, notification_name);
        bump::Observer* update =
            new bump::ObjectObserver<Renderer, unsigned int>(
                this, &Renderer::updateNumRenderPasses,
                "UpdateNumRenderPasses");
        bump::Observer* change_name =
            new bump::ObjectObserver<Renderer, const char*>(
                this, &Renderer::changeName, "ChangeName");
        bump::Observer* change_name_str =
            new bump::ObjectObserver<Renderer, bump::String>(
                this, &Renderer::changeNameWithString, "ChangeNameWithString");
        ADD_OBSERVER(update);
    ...
    void changeName(const char* name) {
        std::cout << bump::String(
                         "- \"%1\" changed name to \"%2\" using const char* "
                         "data type.")
                         .arg(_name, name)
                  << std::endl;
        _name = name;
    ...
    void changeNameWithString(const bump::String& name) {
        std::cout << bump::String(
                         "- \"%1\" changed name to \"%2\" using bump::String "
                         "data type.")
                         .arg(_name, name)
                  << std::endl;
        _name = name;
     ...
     */
    unsigned int numRenderPasses() { return _numRenderPasses; }

    /**
     * Increments the redraw request count.
     */
    void requestRedraw() {
        std::cout << bump::String("- %1 requesting redraw").arg(_name)
                  << std::endl;
        ++_redrawRequestCount;
    }
     ...
     * @return the request redraw count.
     */
    unsigned int requestRedrawCount() { return _redrawRequestCount; }
    ...
    void updateNumRenderPasses(unsigned int numRenderPasses) {
        std::cout << bump::String("- %1 update numRenderingPasses: %2")
                         .arg(_name, _numRenderPasses)
                  << std::endl;
 ...
 * convenience macro
 *
 * And that's all there is to it! This example shows off some do's and don't's
 * for passing notifications with and without objects. It also demonstrates what
 * will happen if you don't pass objects with the correct type.
 *
 * The most important thing to remember when using the NotificationCenter is to
 * make sure to remove your observers from the NotificationCenter in the
 * observer object's destructor. If you don't do this, you could potentially be
 * sending notifications to NULL which could crash your application. One
...
int main() {
    ...
    // If you don't cast the int to an unsigned int, the notification center
    // will throw an exception because the type does not match properly. This is
    // important as it ensures you object passes through the system and comes
    // out the other end properly.
    std::cout << "\nPosting \"UpdateNumRenderPasses\" notification incorrectly"
              << std::endl;
    try {
