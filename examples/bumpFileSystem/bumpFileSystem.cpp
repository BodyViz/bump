...
void joinPathsDemo() {
    ...
    //=========================================================================
    // Sometimes you need to join file paths together. The bump::FileSystem API
    // is very helpful when it comes to this task. It supports joining up to 9
    // individual strings together into a single path.
    //=========================================================================
    ...
    path = bump::FileSystem::join(bump::FileSystem::currentPath(),
                                  "example_file.txt");
    std::cout << "- \"" << path << "\"" << std::endl;
}
...
void systemPathsDemo() {
    ...
    //=========================================================================
    // The system paths methods all you to query the file system for the current
    // path (a.k.a. current working directory) as well as the temporary path set
    // for the file system. These methods can come in very handy for certain
    // situations.
    //=========================================================================
    ...
    bump::String new_current_path =
        bump::FileSystem::join(current_path, "Test Directory");
...
void pathQueriesDemo() {
    ...
    //=========================================================================
    // The path query methods are just quick ways to figure out if the file
    // system object exists or what type of object it actually is. You can also
    // use the bump::FileInfo class to get this information as well. The methods
    // are provided in both locations simply for convenience.
    //=========================================================================
    ...
    std::cout << "- Is Directory:     " << (is_directory ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is File:          " << (is_file ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is Symlink:       " << (is_symlink ? "YES" : "NO")
              << std::endl;
...
void directoriesDemo() {
    ...
    //=========================================================================
    // The directories methods allow you to create and remove directories as
    // well as copy and rename them. These are very standard operations. In
    // addition to these basic methods, the bump::FileSystem API also allows you
    // to remove and copy directories that aren't already empty. These can be
    // incredibly useful in many situations. Finally, there are also directory
    // listing methods allowing you to get a list of paths or bump::FileInfo
    ...
    std::cout
        << "- Renamed Directory: \"Test Directory\" to \"Renamed Directory\""
        << std::endl;

    // Copy the directory
    bump::FileSystem::copyDirectory("Renamed Directory", "Copied Directory");
    std::cout
        << "- Copied Directory:  \"Renamed Directory\" to \"Copied Directory\""
        << std::endl;
    ...
    std::cout << "- Created File:      \"Test Directory/paper.doc\""
              << std::endl;
    bump::FileSystem::createFile("Test Directory/help.pdf");
    std::cout << "- Created File:      \"Test Directory/help.pdf\""
              << std::endl;

    // List all the files inside the directory
    std::cout << "- Items inside       \"Test Directory\":" << std::endl;
    bump::StringList directory_items =
        bump::FileSystem::directoryList("Test Directory");
    BOOST_FOREACH (const bump::String& directory_item, directory_items) {
        std::cout << "\t- Relative Path: \"" << directory_item << "\""
                  << std::endl;
        std::cout << "\t- Absolute Path: \""
                  << bump::FileInfo(directory_item).absolutePath() << "\""
                  << std::endl;
    }

    // Copy the directory and it's contents
    bump::FileSystem::copyDirectoryAndContents("Test Directory",
                                               "Copied Directory");
    std::cout << "- Copied Directory and Contents: \"Test Directory\" to "
                 "\"Copied Directory\""
              << std::endl;

    // Remove the directories and all their contents
    bump::FileSystem::removeDirectoryAndContents("Test Directory");
    std::cout << "- Removed Directory and Contents: \"Test Directory\""
              << std::endl;
    bump::FileSystem::removeDirectoryAndContents("Copied Directory");
    std::cout << "- Removed Directory and Contents: \"Copied Directory\""
              << std::endl;
}
...
void filesDemo() {
    ...
    //=========================================================================
    // The files methods allow you to create and remove files as well as copy
    // and rename them.
    //=========================================================================
    ...
    bump::FileSystem::renameFile(".hidden_file.doc",
                                 ".hidden_file_renamed.doc");
    std::cout << "- Renamed File: \"output.txt\" to \"output_renamed.txt\""
              << std::endl;
    std::cout << "- Renamed File: \".hidden_file.doc\" to "
                 "\".hidden_file_renamed.doc\""
              << std::endl;
    ...
    std::cout << "- Copied File:  \"output_renamed.txt\" to \"output.txt\""
              << std::endl;
    std::cout << "- Copied File:  \".hidden_file_renamed.doc\" to "
                 "\".hidden_file.doc\""
              << std::endl;
...
void symbolicLinksDemo() {
    ...
    //=========================================================================
    // The symbolic link methods allow you to create and remove files as well as
    // copy and rename them.
    //=========================================================================

    // Create a file symbolic link
    bump::FileSystem::createFile("output.txt");
    bump::FileSystem::createFileSymbolicLink("output.txt",
                                             "output_symlink.txt");
    std::cout << "- Created File: \"output.txt\"" << std::endl;
    std::cout
        << "- Created File Symlink: \"output_symlink.txt\" -> \"output.txt\""
        << std::endl;
    ...
    std::cout << "- Created Directory Symlink: \"Files Symlink\" -> \"Files\""
              << std::endl;

    // Rename the symbolic links
    bump::FileSystem::renameSymbolicLink("output_symlink.txt",
                                         "output_symlink_renamed.txt");
    bump::FileSystem::renameSymbolicLink("Files Symlink",
                                         "Files Symlink Renamed");
    std::cout
        << "- Renamed: \"output_symlink.txt\" to \"output_symlink_renamed.txt\""
        << std::endl;
    std::cout << "- Renamed: \"Files Symlink\" to \"Files Symlink Renamed\""
              << std::endl;

    // Copy the symbolic links
    bump::FileSystem::copySymbolicLink("output_symlink_renamed.txt",
                                       "output_symlink_copied.txt");
    bump::FileSystem::copySymbolicLink("Files Symlink Renamed",
                                       "Files Symlink Copied");
    std::cout << "- Renamed: \"output_symlink_renamed.txt\" to "
                 "\"output_symlink_copied.txt\""
              << std::endl;
    std::cout
        << "- Renamed: \"Files Symlink Renamed\" to \"Files Symlink Copied\""
        << std::endl;
    ...
    std::cout << "- Removed Symlink: \"output_symlink_renamed.txt\""
              << std::endl;
    std::cout << "- Removed Symlink: \"output_symlink_copied.txt\""
              << std::endl;
...
void permissionsDemo() {
    // Only run the permissions demo logic when not on Windows. None of the
    // permissions methods are currently implemented on the Windows OS.
    ...
    //=========================================================================
    // The permissions methods allow you to read all the permissions in regards
    // to a file, directory or symbolic link. Additionally, you can modify the
    // permissions using the bump::FileSystem API.
    //=========================================================================
    ...
    bump::FileSystem::Permissions permissions =
        bump::FileSystem::permissions("output.txt");
    std::cout << "- Is readable by owner: "
              << (permissions & bump::FileSystem::OWNER_READ ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is writable by owner: "
              << (permissions & bump::FileSystem::OWNER_WRITE ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is executable by owner: "
              << (permissions & bump::FileSystem::OWNER_EXE ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is readable by group: "
              << (permissions & bump::FileSystem::GROUP_READ ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is writable by group: "
              << (permissions & bump::FileSystem::GROUP_WRITE ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is executable by group: "
              << (permissions & bump::FileSystem::GROUP_EXE ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is readable by others: "
              << (permissions & bump::FileSystem::OTHERS_READ ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is writable by others: "
              << (permissions & bump::FileSystem::OTHERS_WRITE ? "YES" : "NO")
              << std::endl;
    std::cout << "- Is executable by others: "
              << (permissions & bump::FileSystem::OTHERS_EXE ? "YES" : "NO")
              << std::endl;

    // Read the permissions using the bump::FileInfo convenience methods
    std::cout << "\nPermissions using bump::FileInfo API for \"output.txt\""
              << std::endl;
    bump::FileInfo file_info("output.txt");
    std::cout << "- Is readable by owner: "
              << (file_info.isReadableByOwner() ? "YES" : "NO") << std::endl;
    std::cout << "- Is writable by owner: "
              << (file_info.isWritableByOwner() ? "YES" : "NO") << std::endl;
    std::cout << "- Is executable by owner: "
              << (file_info.isExecutableByOwner() ? "YES" : "NO") << std::endl;
    std::cout << "- Is readable by group: "
              << (file_info.isReadableByGroup() ? "YES" : "NO") << std::endl;
    std::cout << "- Is writable by group: "
              << (file_info.isWritableByGroup() ? "YES" : "NO") << std::endl;
    std::cout << "- Is executable by group: "
              << (file_info.isExecutableByGroup() ? "YES" : "NO") << std::endl;
    std::cout << "- Is readable by others: "
              << (file_info.isReadableByOthers() ? "YES" : "NO") << std::endl;
    std::cout << "- Is writable by others: "
              << (file_info.isWritableByOthers() ? "YES" : "NO") << std::endl;
    std::cout << "- Is executable by others: "
              << (file_info.isExecutableByOthers() ? "YES" : "NO") << std::endl;

    // Set the permissions using the bump::FileSystem setPermissions method
    bump::FileSystem::Permissions new_permissions =
        (bump::FileSystem::OWNER_ALL | bump::FileSystem::GROUP_READ |
    ...
    // Set the permissions using the bump::FileSystem convenience methods. These
    // are useful if you just want to change a single permission without
    // changing any of the other permissions. If you want to change all the
    // permissions, then the bump::FileSystem::setPermissions() method is what
    // you want to use.
    ...
    std::cout
        << "\nChanged permissions for \"output.txt\" using convenience methods"
        << std::endl;
    std::cout << "- Is readable by owner: "
              << (file_info.isReadableByOwner() ? "YES" : "NO") << std::endl;
    std::cout << "- Is writable by owner: "
...
void datesDemo() {
    ...
    //=========================================================================
    // The dates methods allow you to read the modified date of a file system
    // object as well as set it.
    //=========================================================================
 ...
 *
 * The FileSystem namespace is used to manipulate file system objects such as
 * files, directories and symbolic links. It is also used to query information
 * about these types of objects, rename them, copy them and even recursively
 * remove them.
 *
 ...
 *    - Symbolic Links (createSymbolicLink, removeSymbolicLink,
 * renameSymbolicLink, etc.)
 *    - Permissions (setPermissions, permissions, setIsReadableByUser,
