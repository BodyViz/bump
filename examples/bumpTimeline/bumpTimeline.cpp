...
/**
 * Renders out asterisks as an animation to std::cout based on the timeline
 * properties.
 */
void executeAnimation(bump::Timeline* timeline, bool pauseAtMiddle) {
    ...
    // Now continuously keep updating the timeline and render an asterisk each
    // time the timeline hits a 1/80th increment
    int current_index = 1;
    while (timeline->state() == bump::Timeline::RUNNING) {
        ...
        if (timeline->stepValue() >=
            (double)current_index / TOTAL_ANIMATION_CHARACTERS) {
            std::cout << "*" << std::flush;
            ++current_index;

            // Pause the animation in the middle if specified
            if (pauseAtMiddle &&
                current_index == TOTAL_ANIMATION_CHARACTERS / 2) {
                timeline->pause();
 ...
 *
 * The Timeline class is used for controlling animations. It is most commonly
 * used for controlling GUI element animations. In order to use the timeline,
 * you create one, set the parameters such as direction, output range and curve
 * shape, then start the timeline. You then need to periodically call the update
...
int main() {
    ...
    ease_in_and_out_timeline.setCurveShape(
        bump::Timeline::EASE_IN_AND_OUT_CURVE);
