...
int main(int argc, char** argv) {
    ...
    // types.
    std::cout << "\nStarting up the \"doSomeMoreWork\" method with a seconds "
                 "auto timer"
              << std::endl;
    doSomeMoreWork(bump::AutoTimer::SECONDS);
    std::cout << "\nStarting up the \"doSomeMoreWork\" method with a "
                 "milliseconds auto timer"
              << std::endl;
    doSomeMoreWork(bump::AutoTimer::MILLISECONDS);
    std::cout << "\nStarting up the \"doSomeMoreWork\" method with a "
                 "microseconds auto timer"
              << std::endl;
    doSomeMoreWork(bump::AutoTimer::MICROSECONDS);
    std::cout << "\nStarting up the \"doSomeMoreWork\" method with a "
                 "nanoseconds auto timer"
              << std::endl;
    doSomeMoreWork(bump::AutoTimer::NANOSECONDS);

    // If you have a massive method that is all in the same scope, and you need
    // to break down the performance into chunks, you could use heap allocated
    // auto timers to figure it out. Here's an example of doing this.
