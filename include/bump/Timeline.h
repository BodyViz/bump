 ...
 *
 * It is most commonly used for controlling GUI element animations. In order to
 * use the timeline, you create one, set the parameters such as direction,
 * output range and curve shape, then start the timeline. You then need to
 * periodically call the update method. After the timeline has been updated, use
 * the stepValue or stepIncrement values to update the animations.
 */
...
public:
    ...
    enum CurveShape {
        ...
        EASE_IN_CURVE, /**< Timeline moves slow at first, then increases in
                          speed. */
        EASE_OUT_CURVE,       /**< Timeline moves fast at first, then slows down
                                 steadily. */
        EASE_IN_AND_OUT_CURVE /**< Timeline starts slow, increases in speed,
                                 peaks in the middle, then eases out. */
    };
     ...
     * @param curveShape The curve shape defining the acceleration of the
     * timeline output.
     */
    Timeline(double duration, const Direction& direction = FORWARDS,
     ...
     *
     * This method needs to be called each time you wish to update the step
     * value of the timeline. After the timeline has been updated, you will use
     * either the stepValue or stepIncrement functions to update your
     * animations.
     */
    void update();
...
protected:
    ...
    Direction
        _direction; /**< @internal Timeline forwards or backwards direction. */
    CurveShape
        _curveShape;      /**< @internal Timeline curve shape interpolation. */
    ...
    double _acceleration; /**< @internal How fast or slow the timeline
                             accelerates. */
    double _stepValue; /**< @internal The step output value generated from start
                          to end. */
    double _stepIncrement; /**< @internal The step output increment between each
                              update. */
