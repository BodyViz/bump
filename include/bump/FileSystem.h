 ...
 *    - Directories (createDirectory(), removeDirectory(), directoryInfoList(),
 * etc.)
...
// Typedefs
typedef unsigned int Permissions; /**< Defines a Permissions wrapper allowing
                                     Permission objects to be OR'd together. */
...
/**
 * Converts the path to a Windows path by replacing forward slashes with
 * backslashes.
 *
...
/**
 * Converts the path to a Unix path by replacing backslashes with forward
 * slashes.
 *
 ...
 *
 * @code
 *   bump::FileSystem::join("/home/username", "Desktop");          // returns
 * "/home/username/Desktop" bump::FileSystem::join("C:/Program Files", "Visual
 * Studio");  // returns "C:/Program Files/Visual Studio"
 *   bump::FileSystem::join("C:\\Program Files", "Visual Studio"); // returns
...
BUMP_EXPORT String join(const String& path1, const String& path2,
                        const String& path3, const String& path4,
                        const String& path5, const String& path6,
                        const String& path7, const String& path8);
...
/**
 * Removes the specified directory's contents recursively, then removes the
 * directory itself.
 *
 * @param path The path of the directory to remove.
 * @return True if the directory and it's contents were removed successfully,
 * false otherwise.
 */
 ...
 *
 * It is important to not that the contents of the source directory will not be
 * copied to the destination directory using this method.
 *
 * @see copyDirectoryAndContents() can copy a directory along with all it's
 * contents.
 *
 * @param source The source directory to copy.
 * @param destination The destination directory to copy the source directory to.
 * @return True if the source directory was copied successfully, false
 * otherwise.
 */
BUMP_EXPORT bool copyDirectory(const String& source, const String& destination);

/**
 * Copies the source directory and all contents over to the destination
 * directory.
 *
 * @param source The source directory to copy.
 * @param destination The destination directory to copy the source directory to.
 * @return True if the source directory and all contents were copied
 * successfully, false otherwise.
 */
BUMP_EXPORT bool copyDirectoryAndContents(const String& source,
 ...
 * @param destination The destination directory to rename the source directory
 * to.
 * @return True if the source directory was renamed successfully, false
 * otherwise.
 */
BUMP_EXPORT bool renameDirectory(const String& source,
                                 const String& destination);
 ...
 * @return A string list of all the file system object paths contained within
 * the directory.
 */
 ...
 *
 * This method works best and is easiest to use when both source and destination
 * are absolute paths. When they are relative, things are a bit trickier. Make
 * sure that the destination path is relative to the current path. Then the
 * source path needs to be relative to the destination path, not the current
 * path.
 *
 ...
 */
BUMP_EXPORT bool createDirectorySymbolicLink(const String& source,
                                             const String& destination);
 ...
 * @return True if the symbolic link was created successfully, false otherwise.
 */
BUMP_EXPORT bool createFileSymbolicLink(const String& source,
                                        const String& destination);
 ...
 * @param destination The destination symbolic link to copy the source symbolic
 * link to.
 * @return True if the source symbolic link was copied successfully, false
 * otherwise.
 */
BUMP_EXPORT bool copySymbolicLink(const String& source,
                                  const String& destination);
 ...
 * @param destination The destination symbolic link to rename the source
 * symbolic link to.
 * @return True if the source symbolic link was renamed successfully, false
 * otherwise.
 */
BUMP_EXPORT bool renameSymbolicLink(const String& source,
                                    const String& destination);
...
/**
 * Replaces the permissions for the file or directory at path with the ones
 * specified.
 *
 * It is important to note that if path if a symbolic link, on Unix operating
 * systems, the symbolic link will be resolved and the permissions will actually
 * be changed on the file or directory that the symbolic link points to. On
 * Windows, the symbolic link permissions are actually modified. However, this
 * should be a very rare case though since modifying permissions of a symbolic
 * link should not need to happen very often.
 *
 ...
 * @param permissions The permissions to change on the file or directory that
 * path points to.
...
/**
 * Enables/disables the owner readable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the owner writable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the owner executable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the group readable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the group writable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the group executable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the others readable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the others writable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
...
/**
 * Enables/disables the others executable permission's bit for the file or
 * directory at path.
 *
 * @todo Add support for Windows.
 *
