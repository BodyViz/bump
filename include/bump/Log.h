 ...
 *	  - Thread-safe access to the stream buffer and thread-safe logging
 * functions
 *    - Timestamp formatting
 *	  - Disabling the log altogether
 *
 * The following environment variables can be used to configure the log at
 * runtime:
 *	  - BUMP_LOG_ENABLED: Disables the log system if set to any of the
 * following:
 *		  * [ NO | FALSE | NOPE | DISABLE ]
 *	  - BUMP_LOG_FILE: Redirects the output from the log to a specified
 * file:
 *		  * /home/username/output.txt
 *	  - BUMP_LOG_LEVEL: Defines the maximum output level for the log:
 *		  * [ ALWAYS_LVL | ERROR_LVL | WARNING_LVL | INFO_LVL |
 * DEBUG_LVL ]
 *
 * Using the log is straight-forward. There are several different convenience
 * functions provided to make this as simple as possible. The easiest way to use
 * the log is with the following functions:
 *    - bumpALWAYS(message);
 *    - bumpERROR(message);
 *    - bumpWARNING(message);
 *    - bumpINFO(message);
 ...
 *
 * Sometimes, it is useful to be able to prefix your output with something like
 * a namespace, application name, error message, etc. For this purpose, the
...
public:
    ...
    enum LogLevel {
        ...
        ERROR_LVL,   /**< Used for errors that the application cannot recover
                        from. */
    ...
    enum TimestampFormat {
        DATE_TIME_TIMESTAMP, /**< Date and time with format: 2013-01-14
                                10:51:33. */
        DATE_TIME_WITH_AM_PM_TIMESTAMP, /**< Date and time with format:
                                           2013-01-14 10:51:33 AM. */
        TIME_TIMESTAMP, /**< Time with format: 2013-01-14 10:51:33. */
        TIME_WITHOUT_AM_PM_TIMESTAMP /**< Time with format: 2013-01-14 10:51:33
                                        AM. */
    };
     ...
     *
     * @return True if timestamps are displayed in the log messages, false
     * otherwise.
     */
    ...
    /**
     * Sets the output stream to a std::ofstream created from the provided
     * filepath.
     *
     * NOTE: This will not change the current log stream if the file cannot be
     * opened successfully.
     *
    ...
    /**
     * Returns the convenience function mutex used to lock all the convenience
     * functions.
     *
     * @return The convenience function mutex used to lock all the convenience
     * functions.
     */
...
protected:
    ...
    LogLevel
        _logLevel; /**< @internal The log level the log instance is set to. */
    bool _isDateTimeFormatEnabled; /**< @internal Whether the date/time are
                                      tacked onto the log output. */
    TimestampFormat
        _timestampFormat; /**< @internal The timestamp format when enabled. */
    std::ostream* _logStream; /**< @internal The log stream to output to. */
    boost::mutex _mutex;      /**< @internal A boost mutex used to make the log
                                 stream access thread-safe. */
    boost::mutex
        _convenienceFunctionMutex; /**< @internal A boost mutex used to lock all
                                      the convenience functions making them
                                      thread-safe. */
...
private:
     ...
     * Constructor.
     *
     * Initializes the log level by attempting to grab the BUMP_LOG_LEVEL
     * environment variables from the user's environment. Needs to be private to
     * ensure you can only create a single instance.
     */
    Log();
 ...
 *
 * These are the most commonly used logging functions. They automatically use
 * the appropriate log level for the message. The log stream is retrieved, then
 * the message is appended into it, then std::endl is appended onto the stream
 * flushing the buffer.
 */
 ...
 *
 * These functions set the appropriate log level for the message, retrieve the
 * stream, append the message into the stream, then flush the stream. This is
 * useful for when you need to generate multiple log messages on a single line.
 */
...
/** Default log functions that only call std::flush on the std::ostream without
 * adding a newline. */

/**
 * Log functions that append a prefix to the output message.
 *
 * It can sometimes be useful to tack on a prefix to your logging messages.
 * These functions are for exactly that. They automatically set the approprate
 * log level for the message and retrieve the stream. First, the prefix is
 * appended to the stream and then the message. Finally, std::endl is appended
 * onto the stream flushing the buffer with a newline character.
 */
BUMP_EXPORT void bumpALWAYS_P(const bump::String& prefix,
                              const bump::String& message);
BUMP_EXPORT void bumpERROR_P(const bump::String& prefix,
                             const bump::String& message);
BUMP_EXPORT void bumpWARNING_P(const bump::String& prefix,
                               const bump::String& message);
BUMP_EXPORT void bumpINFO_P(const bump::String& prefix,
                            const bump::String& message);
BUMP_EXPORT void bumpDEBUG_P(const bump::String& prefix,
                             const bump::String& message);
BUMP_EXPORT void bumpNEWLINE_P(const bump::String& prefix);

/**
 * Log functions that append a prefix to the output message and only flush the
 * buffer.
 *
 * It can sometimes be useful to tack on a prefix to your logging messages.
 * These functions are for exactly that. They automatically set the approprate
 * log level for the message and retrieve the stream. First, the prefix is
 * appended to the stream and then the message. Finally, the stream's buffer is
 * flushed without adding a newline character to the end. This is useful for
 * when you need to generate multiple log messages on a single line.
 */
BUMP_EXPORT void bumpALWAYS_PF(const bump::String& prefix,
                               const bump::String& message);
BUMP_EXPORT void bumpERROR_PF(const bump::String& prefix,
                              const bump::String& message);
BUMP_EXPORT void bumpWARNING_PF(const bump::String& prefix,
                                const bump::String& message);
BUMP_EXPORT void bumpINFO_PF(const bump::String& prefix,
                             const bump::String& message);
