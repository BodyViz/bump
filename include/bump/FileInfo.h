...
/**
 * The FileInfo class is useful when you need information about a file system
 * object.
 *
 * The various types of file system objects support are files, directories and
 * symbolic links. The four types of methods supported are:
 *     - Path Query (fileSize(), isRelative(), isFile(), etc.)
 *     - Path Decomposition (absolutePath(), basename(), filename(),
 * extension(), etc.)
 *     - Permissions Query (isReadableByUser(), owner(), group(), etc.)
 *     - Date Query (modifiedDate(), etc.)
 *
 * It is important to note that all these methods are read-only. You cannot make
 * changes to file system objects using the FileInfo API. It is merely for
 * investigative purposes.
 *
 * @see The FileSystem class for information on how to modify file system
 * objects.
 */
...
public:
     ...
     * Constructor.
     *
     * @param path The absolute or relative path to the file system object to
     * inspect.
     */
     ...
     *
     * @throw bump::FileSystemError When the path does not exist.
     * @throw bump::FileSystemError When the path does not have permissions
     * allowing the path to be checked.
     *
     * @return True if the path points to an empty directory or empty file,
     * false othwerwise.
     */
    bool isEmpty() const;
     ...
     *
     * If the path is absolute to begin with, the path will not be cleaned of
     * duplicate forwards and backwards slashes or have it's symbolic links
     * resolved. If the path is relative, then the result of this call is the
     * currentPath() joined with the relative path. Therefore, the path returned
     * here is never checked again the exists() method.
     *
     * Important to note that a unix style path is returned.
     *
     * @return The absolute path without cleaning the path or resolving symbolic
     * links.
     */
    String absolutePath() const;

    /**
     * Returns the absolute path by resolving symbolic links, removing dot and
     * dot-dot elements.
     *
     * Important to note that a unix style path is returned.
     *
     ...
     *
     * @code
     *   bump::FileInfo("/home/users/user/output.txt").parentPath(); // returns
     * "/home/users/user"
     * @endcode
     *
     * Important to note that a unix style path is returned.
     ...
     *
     * NOTE: If a relative path was passed into the constructor, then a relative
     * path will be returned. Also, the path returned will be a unix style path.
     *
     * @see The absolutePath() and canonicalPath() methods for alternative
     * queries.
     *
     ...
     *   bump::FileInfo("output.tar.gz").completeBasename(); // returns
     * "output.tar"
     * @endcode
     *
     ...
     *   bump::FileInfo("output.tar.gz").completeExtension(); // returns
     * "tar.gz"
     * @endcode
     *
    ...
    /**
     * Returns whether the path is readable by the current user based on the
     * path's permissions.
     *
     ...
     * @throw bump::FileSystemError When the path does not exist.
     *
     * @return True if the path is readable by the current user, false
     * otherwise.
     */
    bool isReadableByUser() const;

    /**
     * Returns whether the path is writable by the current user based on the
     * path's permissions.
     *
     * @todo Add support for Windows.
     *
     ...
     * @throw bump::FileSystemError When the path does not exist.
     *
     * @return True if the path is writable by the current user, false
     * otherwise.
     */
    bool isWritableByUser() const;

    /**
     * Returns whether the path is executable by the current user based on the
     * path's permissions.
     *
     * @todo Add support for Windows.
     *
     ...
     * @throw bump::FileSystemError When the path does not exist.
     *
     * @return True if the path is executable by the current user, false
     * otherwise.
     */
    bool isExecutableByUser() const;

    /**
     * Returns whether the path is readable by the owner based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is writable by the owner based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is executable by the owner based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is readable by the group based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is writable by the group based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is executable by the group based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is readable by others based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is writable by others based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
    ...
    /**
     * Returns whether the path is executable by others based on the path's
     * permissions.
     *
     * @todo Add support for Windows.
     *
...
protected:
    ...
    boost::filesystem::path _path; /**< @internal The boost "path" used to
                                      support the FileInfo API. */
};

// Typedefs
typedef std::vector<FileInfo>
