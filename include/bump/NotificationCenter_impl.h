...
template <class T>
inline KeyObserver<T>::KeyObserver(T* observer, void (T::*functionPointer)(),
...
inline ObjectObserver<T1, T2>::ObjectObserver(T1* observer,
...
inline void ObjectObserver<T1, T2>::notify(const boost::any& object) {
    try {
        ...
        String msg = String(
                         "Notification object for \"%1\" has invalid type for "
                         "bound callback.")
                         .arg(_notificationName);
