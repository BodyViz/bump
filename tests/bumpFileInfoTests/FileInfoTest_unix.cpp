...
TEST_F(FileInfoTest, testFileSize) {
    ...
    EXPECT_EQ(0,
              bump::FileInfo("unittest/symlink_files/output.txt").fileSize());
    EXPECT_EQ(121,
              bump::FileInfo("unittest/symlink_files/info.xml").fileSize());

    // Test valid directories
    EXPECT_THROW(bump::FileInfo("unittest/files").fileSize(),
                 bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/symlink_files").fileSize(),
                 bump::FileSystemError);

    // Test valid directory symlink
    EXPECT_THROW(bump::FileInfo("unittest/symlink_directory").fileSize(),
                 bump::FileSystemError);

    // Test invalid paths
    EXPECT_THROW(
        bump::FileInfo("unittest/does/not/exist/output.txt").fileSize(),
        bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/try/again").fileSize(),
                 bump::FileSystemError);
}
...
TEST_F(FileInfoTest, testIsEmpty) {
    ...
    // Test invalid path
    EXPECT_THROW(bump::FileInfo("unittest/not/valid").isEmpty(),
                 bump::FileSystemError);

    // Test unreadable permissions non-empty file (still works properly on
    // files)
    EXPECT_TRUE(bump::FileSystem::setPermissions("unittest/files/info.xml", 0));
    EXPECT_FALSE(bump::FileInfo("unittest/files/info.xml").isEmpty());
    EXPECT_TRUE(bump::FileSystem::setPermissions("unittest/files/info.xml",
                                                 bump::FileSystem::ALL_ALL));

    // Test unreadable permissions empty file (still works properly on files)
    EXPECT_TRUE(
        bump::FileSystem::setPermissions("unittest/files/output.txt", 0));
    EXPECT_TRUE(bump::FileInfo("unittest/files/output.txt").isEmpty());
    EXPECT_TRUE(bump::FileSystem::setPermissions("unittest/files/output.txt",
                                                 bump::FileSystem::ALL_ALL));

    // Test unreadable permissions directory
    EXPECT_TRUE(bump::FileSystem::setPermissions("unittest/files", 0));
    EXPECT_THROW(bump::FileInfo("unittest/files").isEmpty(),
                 bump::FileSystemError);
    EXPECT_TRUE(bump::FileSystem::setPermissions("unittest/files",
                                                 bump::FileSystem::ALL_ALL));

    // Test invalid paths
    EXPECT_THROW(bump::FileInfo("unittest/not/valid/output.txt").isEmpty(),
                 bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/not/valid").isEmpty(),
                 bump::FileSystemError);
...
TEST_F(FileInfoTest, testOwner) {
    ...
    EXPECT_THROW(bump::FileInfo("unittest/not/valid/output.txt").owner(),
                 bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/not/valid").owner(),
                 bump::FileSystemError);
}

TEST_F(FileInfoTest, testOwnerId) {
    // On Unix, the only thing we can do for an owner id is make sure it's
    // greater than or equal to 0

    // Test a file
    unsigned int owner_id =
        bump::FileInfo("unittest/files/output.txt").ownerId();
    ...
    // Test some invalid paths
    EXPECT_THROW(bump::FileInfo("unittest/not/valid/output.txt").ownerId(),
                 bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/not/valid").ownerId(),
                 bump::FileSystemError);
}

TEST_F(FileInfoTest, testGroup) {
    // On Unix, make sure the group is not empty (all we can do since we can't
    // predict what the group will be)
    ...
    // Test some invalid paths
    EXPECT_THROW(bump::FileInfo("unittest/not/valid/output.txt").group(),
                 bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/not/valid").group(),
                 bump::FileSystemError);
}

TEST_F(FileInfoTest, testGroupId) {
    // On Unix, the only thing we can do for a group id is make sure it's
    // greater than or equal to 0

    // Test a file
    unsigned int group_id =
        bump::FileInfo("unittest/files/output.txt").groupId();
    ...
    // Test some invalid paths
    EXPECT_THROW(bump::FileInfo("unittest/not/valid/output.txt").groupId(),
                 bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/not/valid").groupId(),
                 bump::FileSystemError);
}
