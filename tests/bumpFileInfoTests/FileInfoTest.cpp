...
void FileInfoTest::SetUp() {
    ...
    bump::FileSystem::createDirectorySymbolicLink("empty",
                                                  _emptySymlinkDirectory);
    ...
    bump::FileSystem::createDirectorySymbolicLink(
        "funky.directory", "unittest/funky.directory_symlink");

    // unittest/.hidden_directory
    bump::FileSystem::createDirectory("unittest/.hidden_directory");
    bump::FileSystem::createDirectorySymbolicLink(
        ".hidden_directory", "unittest/.hidden_directory_symlink");

    // unittest/.hidden.funky.directory
    bump::FileSystem::createDirectory("unittest/.hidden.funky.directory");
    bump::FileSystem::createDirectorySymbolicLink(
        ".hidden.funky.directory", "unittest/.hidden.funky.directory_symlink");
    ...
    bump::FileSystem::createFileSymbolicLink("../files/info.xml",
                                             "unittest/symlink_files/info.xml");
...
TEST_F(FileInfoTest, testExists) {
    ...
    EXPECT_TRUE(
        bump::FileInfo("unittest/regular_directory/paper.doc").exists());
    EXPECT_TRUE(bump::FileInfo("unittest/regular_directory/help.pdf").exists());
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_directory/paper.doc").exists());
    ...
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/archive.tar.gz").exists());
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/.hidden_file.txt").exists());
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/.nope_hidden_file.txt").exists());
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/nope_output.txt").exists());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/nope_archive.tar.gz").exists());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/.nope_hidden_file.txt")
                     .exists());
...
TEST_F(FileInfoTest, testIsAbsolute) {
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/output.txt").isAbsolute());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/output.txt").isAbsolute());

    // Test some absolute files
    bump::FileInfo info(
        bump::FileInfo("unittest/files/output.txt").absolutePath());
    ...
    EXPECT_TRUE(info.isAbsolute());
    info = bump::FileInfo(
        bump::FileInfo("unittest/regular_directory").absolutePath());
    EXPECT_TRUE(info.isAbsolute());

    // Test some absolute symlinks
    info = bump::FileInfo(
        bump::FileInfo("unittest/symlink_files/output.txt").absolutePath());
    EXPECT_TRUE(info.isAbsolute());
    info = bump::FileInfo(
        bump::FileInfo("unittest/symlink_files/output.txt").absolutePath());
    EXPECT_TRUE(info.isAbsolute());
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/does/not/exist/output.txt").isAbsolute());
...
TEST_F(FileInfoTest, testIsRelative) {
    ...
    // Test some relative symlinks
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/output.txt").isRelative());
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/output.txt").isRelative());

    // Test some absolute files
    bump::FileInfo info(
        bump::FileInfo("unittest/files/output.txt").isRelative());
    ...
    EXPECT_FALSE(info.isAbsolute());
    info = bump::FileInfo(
        bump::FileInfo("unittest/regular_directory").isRelative());
    EXPECT_FALSE(info.isAbsolute());
    ...
    // Test some invalid relative paths
    EXPECT_TRUE(
        bump::FileInfo("unittest/does/not/exist/output.txt").isRelative());
...
TEST_F(FileInfoTest, testIsDirectory) {
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/.hidden_file.txt").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/regular_directory/paper.doc").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/regular_directory/help.pdf").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_directory/paper.doc").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_directory/help.pdf").isDirectory());

    // Test file symlinks
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/output.txt").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/archive.tar.gz").isDirectory());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/.hidden_file.txt")
                     .isDirectory());
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest_does_not_exist/regular").isDirectory());

    // Test invalid directory symlinks
    EXPECT_FALSE(
        bump::FileInfo("unittest_nope/symlink_directory").isDirectory());
    EXPECT_FALSE(bump::FileInfo("unittest/nope_symlink_files").isDirectory());

    // Test invalid files
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/nope_output.txt").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/.nope_hidden_file.txt").isDirectory());

    // Test invalid file symlinks
    EXPECT_FALSE(
        bump::FileInfo("unittest/not_a_directory/paper.doc").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/not_a_directory/help.pdf").isDirectory());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/nope_output.txt").isDirectory());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/nope_archive.tar.gz")
                     .isDirectory());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/.nope_hidden_file.txt")
                     .isDirectory());
...
TEST_F(FileInfoTest, testIsFile) {
    ...
    EXPECT_TRUE(
        bump::FileInfo("unittest/regular_directory/paper.doc").isFile());
    EXPECT_TRUE(bump::FileInfo("unittest/regular_directory/help.pdf").isFile());
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_directory/paper.doc").isFile());
    ...
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/archive.tar.gz").isFile());
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/.hidden_file.txt").isFile());
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/.nope_hidden_file.txt").isFile());
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/nope_output.txt").isFile());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/nope_archive.tar.gz").isFile());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/.nope_hidden_file.txt")
                     .isFile());
...
TEST_F(FileInfoTest, testIsSymbolicLink) {
    ...
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/archive.tar.gz").isSymbolicLink());
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/.hidden_file.txt").isSymbolicLink());
    EXPECT_FALSE(bump::FileInfo("unittest/regular_directory/paper.doc")
                     .isSymbolicLink());
    EXPECT_FALSE(
        bump::FileInfo("unittest/regular_directory/help.pdf").isSymbolicLink());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_directory/paper.doc")
                     .isSymbolicLink());
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_directory/help.pdf").isSymbolicLink());

    // Test file symlinks
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/output.txt").isSymbolicLink());
    EXPECT_TRUE(bump::FileInfo("unittest/symlink_files/archive.tar.gz")
                    .isSymbolicLink());
    EXPECT_TRUE(bump::FileInfo("unittest/symlink_files/.hidden_file.txt")
                    .isSymbolicLink());

    // Test invalid directories
    EXPECT_FALSE(bump::FileInfo("unittest_does_not_exist").isSymbolicLink());
    EXPECT_FALSE(
        bump::FileInfo("unittest_does_not_exist/files").isSymbolicLink());
    EXPECT_FALSE(
        bump::FileInfo("unittest_does_not_exist/regular").isSymbolicLink());

    // Test invalid directory symlinks
    EXPECT_FALSE(
        bump::FileInfo("unittest_nope/symlink_directory").isSymbolicLink());
    EXPECT_FALSE(
        bump::FileInfo("unittest/nope_symlink_files").isSymbolicLink());

    // Test invalid files
    EXPECT_FALSE(
        bump::FileInfo("unittest/files/nope_output.txt").isSymbolicLink());
    EXPECT_FALSE(bump::FileInfo("unittest/files/.nope_hidden_file.txt")
                     .isSymbolicLink());

    // Test invalid file symlinks
    EXPECT_FALSE(
        bump::FileInfo("unittest/not_a_directory/paper.doc").isSymbolicLink());
    EXPECT_FALSE(
        bump::FileInfo("unittest/not_a_directory/help.pdf").isSymbolicLink());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/nope_output.txt")
                     .isSymbolicLink());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/nope_archive.tar.gz")
                     .isSymbolicLink());
    EXPECT_FALSE(bump::FileInfo("unittest/symlink_files/.nope_hidden_file.txt")
...
TEST_F(FileInfoTest, testIsHidden) {
    ...
    // Test file symlink
    EXPECT_FALSE(
        bump::FileInfo("unittest/symlink_files/output.txt").isHidden());

    // Test hidden file symlink
    EXPECT_TRUE(
        bump::FileInfo("unittest/symlink_files/.hidden_file.txt").isHidden());
    ...
    EXPECT_TRUE(bump::FileSystem::createDirectorySymbolicLink(
        ".hidden_files", "unittest/.hidden_files_symlink"));
...
TEST_F(FileInfoTest, testAbsolutePath) {
    // Test file
    bump::String abs_path =
        bump::FileInfo("unittest/files/output.txt").absolutePath();
    EXPECT_FALSE(abs_path.isEmpty());
    bump::String expected_abs_path = bump::FileSystem::join(
        bump::FileSystem::currentPath(), "unittest/files/output.txt");
    ...
    abs_path =
        bump::FileInfo("unittest/symlink_files/output.txt").absolutePath();
    EXPECT_FALSE(abs_path.isEmpty());
    expected_abs_path = bump::FileSystem::join(
        bump::FileSystem::currentPath(), "unittest/symlink_files/output.txt");
    EXPECT_STREQ(expected_abs_path.c_str(), abs_path.c_str());
    ...
    EXPECT_FALSE(abs_path.isEmpty());
    expected_abs_path =
        bump::FileSystem::join(bump::FileSystem::currentPath(), "");
    EXPECT_STREQ(expected_abs_path.c_str(), abs_path.c_str());
}

TEST_F(FileInfoTest, testCanonicalPath) {
    // Test file
    bump::String canonical_path =
        bump::FileInfo("unittest/files/output.txt").canonicalPath();
    EXPECT_FALSE(canonical_path.isEmpty());
    bump::String expected_canonical_path = bump::FileSystem::join(
        bump::FileSystem::currentPath(), "unittest/files/output.txt");
    EXPECT_STREQ(expected_canonical_path.c_str(), canonical_path.c_str());

    // Test file symlink
    canonical_path =
        bump::FileInfo("unittest/symlink_files/output.txt").canonicalPath();
    EXPECT_FALSE(canonical_path.isEmpty());
    expected_canonical_path = bump::FileSystem::join(
        bump::FileSystem::currentPath(), "unittest/files/output.txt");
    EXPECT_STREQ(expected_canonical_path.c_str(), canonical_path.c_str());
    ...
    EXPECT_FALSE(canonical_path.isEmpty());
    expected_canonical_path = bump::FileSystem::join(
        bump::FileSystem::currentPath(), "unittest/files");
    EXPECT_STREQ(expected_canonical_path.c_str(), canonical_path.c_str());

    // Test directory symlink
    canonical_path =
        bump::FileInfo("unittest/symlink_directory").canonicalPath();
    EXPECT_FALSE(canonical_path.isEmpty());
    expected_canonical_path = bump::FileSystem::join(
        bump::FileSystem::currentPath(), "unittest/regular_directory");
    EXPECT_STREQ(expected_canonical_path.c_str(), canonical_path.c_str());

    // Test invalid paths (passes since the path is never resolved)
    EXPECT_THROW(
        bump::FileInfo("unittest/not/valid/output.txt").canonicalPath(),
        bump::FileSystemError);
    EXPECT_THROW(bump::FileInfo("unittest/not/valid").canonicalPath(),
                 bump::FileSystemError);

    // Test an empty path
    EXPECT_STREQ(bump::FileSystem::currentPath().c_str(),
                 bump::FileInfo("").canonicalPath().c_str());
}

TEST_F(FileInfoTest, testParentPath) {
    // Test file
    bump::String parent_path =
        bump::FileInfo("unittest/files/output.txt").parentPath();
    EXPECT_FALSE(parent_path.isEmpty());
    EXPECT_STREQ("unittest/files", parent_path.c_str());

    // Test file symlink
    parent_path =
        bump::FileInfo("unittest/symlink_files/output.txt").parentPath();
    EXPECT_FALSE(parent_path.isEmpty());
...
TEST_F(FileInfoTest, testBasename) {
    ...
    bump::String basename =
        bump::FileInfo("unittest/files/output.txt").basename();
    ...
    basename =
        bump::FileInfo("unittest/symlink_files/archive.tar.gz").basename();
    EXPECT_STREQ("archive", basename.c_str());

    // Test hidden filename file symlink
    basename =
        bump::FileInfo("unittest/symlink_files/.hidden_file.txt").basename();
    EXPECT_STREQ(".hidden_file", basename.c_str());
    ...
    basename =
        bump::FileInfo("unittest/.hidden.funky.directory_symlink").basename();
...
TEST_F(FileInfoTest, testCompleteBasename) {
    // Test regular filename file
    bump::String complete_basename =
        bump::FileInfo("unittest/files/output.txt").completeBasename();
    EXPECT_STREQ("output", complete_basename.c_str());

    // Test compound filename file
    complete_basename =
        bump::FileInfo("unittest/files/archive.tar.gz").completeBasename();
    EXPECT_STREQ("archive.tar", complete_basename.c_str());

    // Test hidden filename file
    complete_basename =
        bump::FileInfo("unittest/files/.hidden_file.txt").completeBasename();
    ...
    // Test compound filename directory
    complete_basename =
        bump::FileInfo("unittest/funky.directory").completeBasename();
    EXPECT_STREQ("funky.directory", complete_basename.c_str());

    // Test hidden filename directory
    complete_basename =
        bump::FileInfo("unittest/.hidden_directory").completeBasename();
    EXPECT_STREQ(".hidden_directory", complete_basename.c_str());

    // Test compound hidden filename directory
    complete_basename =
        bump::FileInfo("unittest/.hidden.funky.directory").completeBasename();
    EXPECT_STREQ(".hidden.funky.directory", complete_basename.c_str());

    // Test regular filename file symlink
    complete_basename =
        bump::FileInfo("unittest/symlink_files/output.txt").completeBasename();
    EXPECT_STREQ("output", complete_basename.c_str());

    // Test compound filename file symlink
    complete_basename = bump::FileInfo("unittest/symlink_files/archive.tar.gz")
                            .completeBasename();
    EXPECT_STREQ("archive.tar", complete_basename.c_str());

    // Test hidden filename file symlink
    complete_basename =
        bump::FileInfo("unittest/symlink_files/.hidden_file.txt")
            .completeBasename();
    EXPECT_STREQ(".hidden_file", complete_basename.c_str());
    ...
    // Test regular filename directory symlink
    complete_basename =
        bump::FileInfo("unittest/empty_symlink").completeBasename();
    EXPECT_STREQ("empty_symlink", complete_basename.c_str());

    // Test compound filename directory symlink
    complete_basename =
        bump::FileInfo("unittest/funky.directory_symlink").completeBasename();
    EXPECT_STREQ("funky.directory_symlink", complete_basename.c_str());

    // Test hidden filename directory symlink
    complete_basename =
        bump::FileInfo("unittest/.hidden_directory_symlink").completeBasename();
    EXPECT_STREQ(".hidden_directory_symlink", complete_basename.c_str());

    // Test compound hidden filename directory symlink
    complete_basename =
        bump::FileInfo("unittest/.hidden.funky.directory_symlink")
            .completeBasename();
    EXPECT_STREQ(".hidden.funky.directory_symlink", complete_basename.c_str());

    // Test an invalid file with an extension
    complete_basename =
        bump::FileInfo("unittest/files/invalid.doc").completeBasename();
    EXPECT_STREQ("invalid", complete_basename.c_str());

    // Test an invalid file without an extension
    complete_basename =
        bump::FileInfo("unittest/files/invalid").completeBasename();
    EXPECT_STREQ("invalid", complete_basename.c_str());
...
TEST_F(FileInfoTest, testExtension) {
    // Test regular filename file
    bump::String extension =
        bump::FileInfo("unittest/files/output.txt").extension();
    ...
    // Test compound filename file symlink
    extension =
        bump::FileInfo("unittest/symlink_files/archive.tar.gz").extension();
    EXPECT_STREQ("gz", extension.c_str());

    // Test hidden filename file symlink
    extension =
        bump::FileInfo("unittest/symlink_files/.hidden_file.txt").extension();
    EXPECT_STREQ("txt", extension.c_str());
    ...
    // Test hidden filename directory symlink
    extension =
        bump::FileInfo("unittest/.hidden_directory_symlink").extension();
    EXPECT_STREQ("", extension.c_str());

    // Test compound hidden filename directory symlink
    extension =
        bump::FileInfo("unittest/.hidden.funky.directory_symlink").extension();
    EXPECT_STREQ("", extension.c_str());
...
TEST_F(FileInfoTest, testCompleteExtension) {
    // Test regular filename file
    bump::String complete_extension =
        bump::FileInfo("unittest/files/output.txt").completeExtension();
    EXPECT_STREQ("txt", complete_extension.c_str());

    // Test compound filename file
    complete_extension =
        bump::FileInfo("unittest/files/archive.tar.gz").completeExtension();
    EXPECT_STREQ("tar.gz", complete_extension.c_str());

    // Test hidden filename file
    complete_extension =
        bump::FileInfo("unittest/files/.hidden_file.txt").completeExtension();
    EXPECT_STREQ("txt", complete_extension.c_str());
    ...
    // Test compound filename directory
    complete_extension =
        bump::FileInfo("unittest/funky.directory").completeExtension();
    EXPECT_STREQ("", complete_extension.c_str());

    // Test hidden filename directory
    complete_extension =
        bump::FileInfo("unittest/.hidden_directory").completeExtension();
    EXPECT_STREQ("", complete_extension.c_str());

    // Test compound hidden filename directory
    complete_extension =
        bump::FileInfo("unittest/.hidden.funky.directory").completeExtension();
    EXPECT_STREQ("", complete_extension.c_str());

    // Test regular filename file symlink
    complete_extension =
        bump::FileInfo("unittest/symlink_files/output.txt").completeExtension();
    EXPECT_STREQ("txt", complete_extension.c_str());

    // Test compound filename file symlink
    complete_extension = bump::FileInfo("unittest/symlink_files/archive.tar.gz")
                             .completeExtension();
    EXPECT_STREQ("tar.gz", complete_extension.c_str());
    ...
    // Test compound hidden filename file symlink
    complete_extension =
        bump::FileInfo("unittest/symlink_files/.archive.tar.gz")
            .completeExtension();
    EXPECT_STREQ("tar.gz", complete_extension.c_str());

    // Test regular filename directory symlink
    complete_extension =
        bump::FileInfo("unittest/empty_symlink").completeExtension();
    EXPECT_STREQ("", complete_extension.c_str());

    // Test compound filename directory symlink
    complete_extension =
        bump::FileInfo("unittest/funky.directory_symlink").completeExtension();
    EXPECT_STREQ("", complete_extension.c_str());

    // Test hidden filename directory symlink
    complete_extension = bump::FileInfo("unittest/.hidden_directory_symlink")
                             .completeExtension();
    EXPECT_STREQ("", complete_extension.c_str());
    ...
    // Test an invalid file with an extension
    complete_extension =
        bump::FileInfo("unittest/files/invalid.doc").completeExtension();
    EXPECT_STREQ("doc", complete_extension.c_str());

    // Test an invalid file without an extension
    complete_extension =
        bump::FileInfo("unittest/files/invalid").completeExtension();
    EXPECT_STREQ("", complete_extension.c_str());
...
TEST_F(FileInfoTest, testFilename) {
    // Test regular filename file
    bump::String filename =
        bump::FileInfo("unittest/files/output.txt").filename();
    ...
    // Test compound filename file symlink
    filename =
        bump::FileInfo("unittest/symlink_files/archive.tar.gz").filename();
    EXPECT_STREQ("archive.tar.gz", filename.c_str());

    // Test hidden filename file symlink
    filename =
        bump::FileInfo("unittest/symlink_files/.hidden_file.txt").filename();
    EXPECT_STREQ(".hidden_file.txt", filename.c_str());
    ...
    // Test compound hidden filename directory symlink
    filename =
        bump::FileInfo("unittest/.hidden.funky.directory_symlink").filename();
