...
public:
    ...
    Renderer(const bump::String& name = "")
        ...
        // Normally we would add the this pointer as an observer to the
     ...
     */
    void changeName(const char* name) { _name = name; }
     ...
     * @param name the new name to apply to the renderer.
     */
    void changeNameWithString(const bump::String& name) { _name = name; }
     ...
     */
    void requestRedraw() { ++_redrawRequestCount; }
...
TEST_F(NotificationTest, testAddObserver1) {
    ...
    bump::Observer* redraw = new bump::KeyObserver<Renderer>(
        _r1, &Renderer::requestRedraw, "RequestRedraw");
    ADD_OBSERVER(redraw);
...
TEST_F(NotificationTest, testAddObserver2) {
    ...
    bump::Observer* update = new bump::ObjectObserver<Renderer, unsigned int>(
        _r1, &Renderer::updateNumRenderPasses, "UpdateNumRenderPasses");
    ADD_OBSERVER(update);
    ...
    unsigned int observers_notified = POST_NOTIFICATION_WITH_OBJECT(
        "UpdateNumRenderPasses", num_render_passes);
    EXPECT_EQ(1, observers_notified);
    EXPECT_EQ(4, _r1->numRenderPasses());

    // Push another notification
    num_render_passes = 8;
    observers_notified = POST_NOTIFICATION_WITH_OBJECT("UpdateNumRenderPasses",
                                                       num_render_passes);
    EXPECT_EQ(1, observers_notified);
...
TEST_F(NotificationTest, testAddObserver3) {
    ...
    bump::Observer* change_name =
        new bump::ObjectObserver<Renderer, const char*>(
            _r1, &Renderer::changeName, "ChangeName");
    ADD_OBSERVER(change_name);
    ...
    unsigned int observers_notified =
        POST_NOTIFICATION_WITH_OBJECT("ChangeName", new_name);
    EXPECT_EQ(1, observers_notified);
...
TEST_F(NotificationTest, testContainsObserver) {
    // Add some observers
    bump::Observer* update1 = new bump::ObjectObserver<Renderer, unsigned int>(
        _r1, &Renderer::updateNumRenderPasses, "UpdateNumRenderPasses");
    bump::Observer* update3 = new bump::ObjectObserver<Renderer, unsigned int>(
        &_r3, &Renderer::updateNumRenderPasses, "UpdateNumRenderPasses");
...
TEST_F(NotificationTest, testPostNotification) {
    ...
    bump::Observer* change_name_str =
        new bump::ObjectObserver<Renderer, bump::String>(
            _r1, &Renderer::changeNameWithString, "ChangeNameWithString");
    ADD_OBSERVER(redraw);
    ADD_OBSERVER(update);
    ADD_OBSERVER(change_name);
    ADD_OBSERVER(change_name_str);
    ...
    redraw = new bump::KeyObserver<Renderer>(_r2, &Renderer::requestRedraw,
                                             "RequestRedraw");
    update = new bump::ObjectObserver<Renderer, unsigned int>(
        _r2, &Renderer::updateNumRenderPasses, "UpdateNumRenderPasses");
    change_name = new bump::ObjectObserver<Renderer, const char*>(
        _r2, &Renderer::changeName, "ChangeName");
    ADD_OBSERVER(redraw);
    ADD_OBSERVER(update);
    ADD_OBSERVER(change_name);
    ...
    change_name = new bump::ObjectObserver<Renderer, const char*>(
        &_r3, &Renderer::changeName, "ChangeName");
    change_name_str = new bump::ObjectObserver<Renderer, bump::String>(
        &_r3, &Renderer::changeNameWithString, "ChangeNameWithString");
    ADD_OBSERVER(change_name);
    ADD_OBSERVER(change_name_str);
    ...
    // Test the change name with string callback (no observers b/c tied to
    // ObjectObserver)
...
TEST_F(NotificationTest, testPostNotificationWithObject) {
    ...
    EXPECT_EQ(0, _r1->requestRedrawCount());
    unsigned int observers_notified =
        POST_NOTIFICATION_WITH_OBJECT("RequestRedraw", 10);
    EXPECT_EQ(0, observers_notified);
    EXPECT_EQ(0, _r1->requestRedrawCount());
    ...
    observers_notified =
        POST_NOTIFICATION_WITH_OBJECT("ChangeNameWithString", new_name_str);
    EXPECT_EQ(2, observers_notified);
    EXPECT_STREQ("Renderer 2", _r1->name().c_str());

    // Test pushing an incorrect type
    EXPECT_THROW(POST_NOTIFICATION_WITH_OBJECT("ChangeNameWithString", 10),
                 bump::NotificationError);
}
