...
void FileSystemTest::SetUp() {
    ...
    bump::FileSystem::createDirectorySymbolicLink("regular_directory",
                                                  _symlinkDirectory);
    ...
    bump::FileSystem::createFileSymbolicLink(
        "../files/output.txt", "unittest/symlink_files/output.txt");
    bump::FileSystem::createFileSymbolicLink(
...
TEST_F(FileSystemTest, testConvertToWindowsPath) {
    ...
    bump::String converted =
        bump::FileSystem::convertToWindowsPath("/home/username/Desktop");
    ...
    // Test a mix
    converted =
        bump::FileSystem::convertToWindowsPath("/home\\username/Desktop\\Test");
    EXPECT_STREQ("\\home\\username\\Desktop\\Test", converted.c_str());
    converted = bump::FileSystem::convertToWindowsPath(
        "C:\\Program Files/Visual Studio\\Test");
...
TEST_F(FileSystemTest, testConvertToUnixPath) {
    ...
    bump::String converted =
        bump::FileSystem::convertToUnixPath("/home/username/Desktop");
    EXPECT_STREQ("/home/username/Desktop", converted.c_str());

    // Test converting a windows path to unix
    converted =
        bump::FileSystem::convertToUnixPath("C:\\Program Files\\Visual Studio");
    EXPECT_STREQ("C:/Program Files/Visual Studio", converted.c_str());

    // Test a mix
    converted =
        bump::FileSystem::convertToUnixPath("/home\\username/Desktop\\Test");
    EXPECT_STREQ("/home/username/Desktop/Test", converted.c_str());
...
TEST_F(FileSystemTest, join3) {
    // Test the default usage
    bump::String joined =
        bump::FileSystem::join("/home", "username", "Desktop");
    EXPECT_STREQ("/home/username/Desktop", joined.c_str());
...
TEST_F(FileSystemTest, join5) {
    // Test the default usage
    bump::String joined =
        bump::FileSystem::join("/", "home", "username", "Desktop", "test");
    ...
    joined = bump::FileSystem::join("C:/", "Program Files", "\\Visual Studio",
                                    "Applications", "/x86_64");
    EXPECT_STREQ("C:/Program Files/Visual Studio/Applications/x86_64",
                 joined.c_str());
...
TEST_F(FileSystemTest, join6) {
    // Test the default usage
    bump::String joined = bump::FileSystem::join("/", "home", "username",
                                                 "Desktop", "test", "default");
    EXPECT_STREQ("/home/username/Desktop/test/default", joined.c_str());
    joined = bump::FileSystem::join("C:/", "Program Files", "\\Visual Studio",
                                    "Applications", "/x86_64", "VC6");
    EXPECT_STREQ("C:/Program Files/Visual Studio/Applications/x86_64/VC6",
                 joined.c_str());
    ...
    joined =
        bump::FileSystem::join("/", "opt/", "local", "/sbin", "app", "/bundle");
...
TEST_F(FileSystemTest, join7) {
    // Test the default usage
    bump::String joined = bump::FileSystem::join(
        "/", "home", "username", "Desktop", "test", "default", "folder");
    ...
    joined = bump::FileSystem::join("C:/", "Program Files", "\\Visual Studio",
                                    "Applications", "/x86_64", "VC6", "Temp");
    EXPECT_STREQ("C:/Program Files/Visual Studio/Applications/x86_64/VC6/Temp",
                 joined.c_str());
    ...
    joined =
        bump::FileSystem::join("/", "", "", "testing", "again", "", "/exe");
    ...
    joined = bump::FileSystem::join("/home", "/username/", "Desktop", "/local",
                                    "app", "/bundle", "/exe/");
    EXPECT_STREQ("/home/username/Desktop/local/app/bundle/exe/",
                 joined.c_str());
    joined = bump::FileSystem::join("/", "opt/", "local", "/sbin", "app",
                                    "/bundle", "exe");
...
TEST_F(FileSystemTest, join8) {
    // Test the default usage
    bump::String joined = bump::FileSystem::join(
        "/", "home", "username", "Desktop", "test", "default", "folder", "sa");
    EXPECT_STREQ("/home/username/Desktop/test/default/folder/sa",
                 joined.c_str());
    joined = bump::FileSystem::join("/", "usr", "/local", "sbin", "app",
                                    "bundle", "exe", "sa");
    EXPECT_STREQ("/usr/local/sbin/app/bundle/exe/sa", joined.c_str());
    joined =
        bump::FileSystem::join("C:/", "Program Files", "\\Visual Studio",
                               "Applications", "/x86_64", "VC6", "Temp", "sa");
    EXPECT_STREQ(
        "C:/Program Files/Visual Studio/Applications/x86_64/VC6/Temp/sa",
        joined.c_str());
    ...
    joined = bump::FileSystem::join("/home", "/username/", "Desktop", "/local",
                                    "app", "/bundle", "/exe/", "so");
    EXPECT_STREQ("/home/username/Desktop/local/app/bundle/exe/so",
                 joined.c_str());
    joined = bump::FileSystem::join("/", "opt/", "local", "/sbin", "app",
                                    "/bundle", "exe", "/so");
...
TEST_F(FileSystemTest, join9) {
    ...
    bump::String joined =
        bump::FileSystem::join("/", "home", "username", "Desktop", "test",
                               "default", "folder", "sa", "temp");
    EXPECT_STREQ("/home/username/Desktop/test/default/folder/sa/temp",
                 joined.c_str());
    joined = bump::FileSystem::join("/", "usr", "/local", "sbin", "app",
                                    "bundle", "exe", "sa", "temp");
    ...
    joined = bump::FileSystem::join("C:/", "Program Files", "\\Visual Studio",
                                    ...
                                    "sa", "aa");
    EXPECT_STREQ(
        "C:/Program Files/Visual Studio/Applications/x86_64/VC6/Temp/sa/aa",
        joined.c_str());
    ...
    // Test the cases with extra forward slashes
    joined = bump::FileSystem::join("/home", "/username/", "Desktop", "/local",
                                    "app", "/bundle", "/exe/", "so", "/temp/");
    EXPECT_STREQ("/home/username/Desktop/local/app/bundle/exe/so/temp/",
                 joined.c_str());
    joined = bump::FileSystem::join("/", "opt/", "local", "/sbin", "app",
                                    "/bundle", "exe", "/so", "/temp");
...
TEST_F(FileSystemTest, testSetCurrentPath) {
    ...
    bump::String new_current_path =
        bump::FileSystem::join(current_path, _unittestDirectory);
    bump::FileSystem::setCurrentPath(new_current_path);
    EXPECT_STREQ(new_current_path.c_str(),
                 bump::FileSystem::currentPath().c_str());

    // Try to set the current path to one that isn't valid
    new_current_path =
        bump::FileSystem::join(current_path, "this_is_not_valid");
    EXPECT_FALSE(bump::FileSystem::setCurrentPath(new_current_path));
}

TEST_F(FileSystemTest, testCurrentPath) {
    // NOTE: We can't test the actual value here since it will change from
    // computer to computer. Therefore, the tests are more abstract.
...
TEST_F(FileSystemTest, testTemporaryPath) {
    // NOTE: We can't test the actual value here since it will change from
    // computer to computer and operating system to operating system. Therefore,
    // the tests are more abstract.
...
TEST_F(FileSystemTest, testExists) {
    ...
    EXPECT_TRUE(
        bump::FileSystem::exists("unittest/regular_directory/paper.doc"));
    EXPECT_TRUE(
        bump::FileSystem::exists("unittest/regular_directory/help.pdf"));
    EXPECT_TRUE(
        bump::FileSystem::exists("unittest/symlink_directory/paper.doc"));
    EXPECT_TRUE(
        bump::FileSystem::exists("unittest/symlink_directory/help.pdf"));

    // Test valid file symlinks
    EXPECT_TRUE(bump::FileSystem::exists("unittest/symlink_files/output.txt"));
    EXPECT_TRUE(
        bump::FileSystem::exists("unittest/symlink_files/archive.tar.gz"));
    EXPECT_TRUE(
        bump::FileSystem::exists("unittest/symlink_files/.hidden_file.txt"));
    ...
    EXPECT_FALSE(
        bump::FileSystem::exists("unittest/files/.nope_hidden_file.txt"));

    // Test invalid file symlinks
    EXPECT_FALSE(
        bump::FileSystem::exists("unittest/not_a_directory/paper.doc"));
    EXPECT_FALSE(bump::FileSystem::exists("unittest/not_a_directory/help.pdf"));
    EXPECT_FALSE(
        bump::FileSystem::exists("unittest/symlink_files/nope_output.txt"));
    EXPECT_FALSE(
        bump::FileSystem::exists("unittest/symlink_files/nope_archive.tar.gz"));
    EXPECT_FALSE(bump::FileSystem::exists(
        "unittest/symlink_files/.nope_hidden_file.txt"));
...
TEST_F(FileSystemTest, testIsDirectory) {
    ...
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/files/archive.tar.gz"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/files/.hidden_file.txt"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/regular_directory/paper.doc"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/regular_directory/help.pdf"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/symlink_directory/paper.doc"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/symlink_directory/help.pdf"));

    // Test file symlinks
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/symlink_files/output.txt"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/symlink_files/archive.tar.gz"));
    EXPECT_FALSE(bump::FileSystem::isDirectory(
        "unittest/symlink_files/.hidden_file.txt"));

    // Test invalid directories
    EXPECT_FALSE(bump::FileSystem::isDirectory("unittest_does_not_exist"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest_does_not_exist/files"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest_does_not_exist/regular"));

    // Test invalid directory symlinks
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest_nope/symlink_directory"));
    EXPECT_FALSE(bump::FileSystem::isDirectory("unittest/nope_symlink_files"));

    // Test invalid files
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/files/nope_output.txt"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/files/.nope_hidden_file.txt"));

    // Test invalid file symlinks
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/not_a_directory/paper.doc"));
    EXPECT_FALSE(
        bump::FileSystem::isDirectory("unittest/not_a_directory/help.pdf"));
    EXPECT_FALSE(bump::FileSystem::isDirectory(
        "unittest/symlink_files/nope_output.txt"));
    EXPECT_FALSE(bump::FileSystem::isDirectory(
        "unittest/symlink_files/nope_archive.tar.gz"));
    EXPECT_FALSE(bump::FileSystem::isDirectory(
        "unittest/symlink_files/.nope_hidden_file.txt"));
...
TEST_F(FileSystemTest, testIsFile) {
    ...
    EXPECT_TRUE(
        bump::FileSystem::isFile("unittest/regular_directory/paper.doc"));
    EXPECT_TRUE(
        bump::FileSystem::isFile("unittest/regular_directory/help.pdf"));
    EXPECT_TRUE(
        bump::FileSystem::isFile("unittest/symlink_directory/paper.doc"));
    EXPECT_TRUE(
        bump::FileSystem::isFile("unittest/symlink_directory/help.pdf"));

    // Test file symlinks
    EXPECT_TRUE(bump::FileSystem::isFile("unittest/symlink_files/output.txt"));
    EXPECT_TRUE(
        bump::FileSystem::isFile("unittest/symlink_files/archive.tar.gz"));
    EXPECT_TRUE(
        bump::FileSystem::isFile("unittest/symlink_files/.hidden_file.txt"));
    ...
    EXPECT_FALSE(
        bump::FileSystem::isFile("unittest/files/.nope_hidden_file.txt"));

    // Test invalid file symlinks
    EXPECT_FALSE(
        bump::FileSystem::isFile("unittest/not_a_directory/paper.doc"));
    EXPECT_FALSE(bump::FileSystem::isFile("unittest/not_a_directory/help.pdf"));
    EXPECT_FALSE(
        bump::FileSystem::isFile("unittest/symlink_files/nope_output.txt"));
    EXPECT_FALSE(
        bump::FileSystem::isFile("unittest/symlink_files/nope_archive.tar.gz"));
...
TEST_F(FileSystemTest, testIsSymbolicLink) {
    ...
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest/files/archive.tar.gz"));
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest/files/.hidden_file.txt"));
    EXPECT_FALSE(bump::FileSystem::isSymbolicLink(
        "unittest/regular_directory/paper.doc"));
    EXPECT_FALSE(bump::FileSystem::isSymbolicLink(
        "unittest/regular_directory/help.pdf"));
    EXPECT_FALSE(bump::FileSystem::isSymbolicLink(
        "unittest/symlink_directory/paper.doc"));
    EXPECT_FALSE(bump::FileSystem::isSymbolicLink(
        "unittest/symlink_directory/help.pdf"));

    // Test file symlinks
    EXPECT_TRUE(
        bump::FileSystem::isSymbolicLink("unittest/symlink_files/output.txt"));
    EXPECT_TRUE(bump::FileSystem::isSymbolicLink(
        "unittest/symlink_files/archive.tar.gz"));
    EXPECT_TRUE(bump::FileSystem::isSymbolicLink(
        "unittest/symlink_files/.hidden_file.txt"));
    ...
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest_does_not_exist/files"));
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest_does_not_exist/regular"));

    // Test invalid directory symlinks
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest_nope/symlink_directory"));
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest/nope_symlink_files"));

    // Test invalid files
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest/files/nope_output.txt"));
    EXPECT_FALSE(bump::FileSystem::isSymbolicLink(
        "unittest/files/.nope_hidden_file.txt"));

    // Test invalid file symlinks
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest/not_a_directory/paper.doc"));
    EXPECT_FALSE(
        bump::FileSystem::isSymbolicLink("unittest/not_a_directory/help.pdf"));
    EXPECT_FALSE(bump::FileSystem::isSymbolicLink(
        "unittest/symlink_files/nope_output.txt"));
    EXPECT_FALSE(bump::FileSystem::isSymbolicLink(
        "unittest/symlink_files/nope_archive.tar.gz"));
...
TEST_F(FileSystemTest, testRemoveDirectory) {
    ...
    EXPECT_FALSE(
        bump::FileSystem::removeDirectory("unittest/does not exist/either"));
    EXPECT_FALSE(bump::FileSystem::removeDirectory(
        "unittest/does not exist/either/for good measure"));

    // Try to remove some files
    EXPECT_FALSE(
        bump::FileSystem::removeDirectory("unittest/files/output.txt"));
    EXPECT_FALSE(
        bump::FileSystem::removeDirectory("unittest/files/archive.tar.gz"));
    EXPECT_FALSE(
        bump::FileSystem::removeDirectory("unittest/files/.hidden_file.txt"));
}
...
TEST_F(FileSystemTest, testRemoveDirectoryAndContents) {
    ...
    // Remove some empty, valid directories
    EXPECT_TRUE(
        bump::FileSystem::removeDirectoryAndContents("unittest/empty_dir1"));
    EXPECT_TRUE(
        bump::FileSystem::removeDirectoryAndContents("unittest/empty_dir2"));

    // Remove the symlink directory
    EXPECT_TRUE(
        bump::FileSystem::removeDirectoryAndContents(_symlinkDirectory));

    // Remove some valid directories and contain files
    EXPECT_TRUE(
        bump::FileSystem::removeDirectoryAndContents(_symlinkFilesDirectory));
    EXPECT_TRUE(
        bump::FileSystem::removeDirectoryAndContents(_regularDirectory));

    // Try to remove some invalid directories
    EXPECT_FALSE(bump::FileSystem::removeDirectoryAndContents(
        "unittest/does not exist"));
    EXPECT_FALSE(bump::FileSystem::removeDirectoryAndContents(
        "unittest/does not exist/either"));
    EXPECT_FALSE(bump::FileSystem::removeDirectoryAndContents(
        "unittest/does not exist/either/for good measure"));

    // Try to remove some files
    EXPECT_FALSE(bump::FileSystem::removeDirectoryAndContents(
        "unittest/files/output.txt"));
    EXPECT_FALSE(bump::FileSystem::removeDirectoryAndContents(
        "unittest/files/archive.tar.gz"));
    EXPECT_FALSE(bump::FileSystem::removeDirectoryAndContents(
        "unittest/files/.hidden_file.txt"));
}
...
TEST_F(FileSystemTest, testRenameDirectory) {
    ...
    EXPECT_TRUE(bump::FileSystem::renameDirectory("unittest/empty_dir1",
                                                  "unittest/renamed_dir1"));
    EXPECT_TRUE(bump::FileSystem::renameDirectory("unittest/empty_dir2",
                                                  "unittest/renamed_dir2"));

    // Try to rename a source directory that doesn't exist
    EXPECT_FALSE(bump::FileSystem::renameDirectory(
        "unittest/not actual directory", "unittest/worth a try"));
    EXPECT_FALSE(bump::FileSystem::renameDirectory("unittest/invalid/dir",
                                                   "unittest/nope"));

    // Try to rename a valid source directory to a destination who's parent
    // directory doesn't exist
    EXPECT_FALSE(bump::FileSystem::renameDirectory(
        "unittest/renamed_dir1", "unittest/long/path/renamed_dir1"));
    EXPECT_FALSE(bump::FileSystem::renameDirectory(
        "unittest/renamed_dir2", "unittest/this/is/not/valid/renamed_dir2"));
}
...
TEST_F(FileSystemTest, testDirectoryList) {
    ...
    bump::StringList empty_dir_list1 =
        bump::FileSystem::directoryList("unittest/empty_dir1");
    EXPECT_TRUE(empty_dir_list1.empty());
    bump::StringList empty_dir_list2 =
        bump::FileSystem::directoryList("unittest/empty_dir2");
    EXPECT_TRUE(empty_dir_list2.empty());

    // Get the directory list of unittest
    bump::StringList unittest_list =
        bump::FileSystem::directoryList("unittest");
    ...
    bump::StringList files_list =
        bump::FileSystem::directoryList("unittest/files");
    ...
    bump::StringList symlink_dir_list =
        bump::FileSystem::directoryList("unittest/symlink_directory");
    EXPECT_EQ(2, symlink_dir_list.size());
    EXPECT_STREQ("unittest/symlink_directory/help.pdf",
                 symlink_dir_list.at(0).c_str());
    EXPECT_STREQ("unittest/symlink_directory/paper.doc",
                 symlink_dir_list.at(1).c_str());
}
...
TEST_F(FileSystemTest, testDirectoryInfoList) {
    ...
    // Get the directory list (should be completely empty)
    bump::FileInfoList empty_dir_list1 =
        bump::FileSystem::directoryInfoList("unittest/empty_dir1");
    EXPECT_TRUE(empty_dir_list1.empty());
    bump::FileInfoList empty_dir_list2 =
        bump::FileSystem::directoryInfoList("unittest/empty_dir2");
    EXPECT_TRUE(empty_dir_list2.empty());

    // Get the directory list of unittest
    bump::FileInfoList unittest_list =
        bump::FileSystem::directoryInfoList("unittest");
    EXPECT_EQ(6, unittest_list.size());
    ...
    EXPECT_STREQ("unittest/regular_directory",
                 unittest_list.at(3).path().c_str());
    EXPECT_STREQ("unittest/symlink_directory",
                 unittest_list.at(4).path().c_str());
    EXPECT_STREQ("unittest/symlink_files", unittest_list.at(5).path().c_str());

    // Get the directory list of the unittest/files directory
    bump::FileInfoList files_list =
        bump::FileSystem::directoryInfoList("unittest/files");
    EXPECT_EQ(3, files_list.size());
    EXPECT_STREQ("unittest/files/.hidden_file.txt",
                 files_list.at(0).path().c_str());
    EXPECT_STREQ("unittest/files/archive.tar.gz",
                 files_list.at(1).path().c_str());
    EXPECT_STREQ("unittest/files/output.txt", files_list.at(2).path().c_str());

    // Get the directory list of the unittest/symlink_directory
    bump::FileInfoList symlink_dir_list =
        bump::FileSystem::directoryInfoList("unittest/symlink_directory");
    EXPECT_EQ(2, symlink_dir_list.size());
    EXPECT_STREQ("unittest/symlink_directory/help.pdf",
                 symlink_dir_list.at(0).path().c_str());
    EXPECT_STREQ("unittest/symlink_directory/paper.doc",
                 symlink_dir_list.at(1).path().c_str());
}
...
TEST_F(FileSystemTest, testCreateDirectorySymbolicLink) {
    ...
    EXPECT_TRUE(bump::FileSystem::createDirectorySymbolicLink(relative_source,
                                                              destination));
    EXPECT_TRUE(bump::FileSystem::exists(destination));
    ...
    EXPECT_TRUE(
        bump::FileSystem::exists(bump::FileInfo(destination).canonicalPath()));

    // Test an incorrectly built relative path directory symlink. It is really
    // important to note that all the tests will pass except for the last one.
    // We get the canonical path to the symlink's target to see if the symlink
    // is actually valid.
    source = "unittest/files/testing";
    destination = "unittest/regular_directory/copied_testing_again";
    EXPECT_TRUE(
        bump::FileSystem::createDirectorySymbolicLink(source, destination));
    EXPECT_TRUE(bump::FileSystem::exists(destination));
    EXPECT_TRUE(bump::FileSystem::isSymbolicLink(destination));
    EXPECT_TRUE(bump::FileSystem::exists(source));
    EXPECT_TRUE(bump::FileSystem::isDirectory(source));
    EXPECT_THROW(bump::FileInfo(destination).canonicalPath(),
                 bump::FileSystemError);

    // Try to create some symlink directories with bad source paths (creation
    // will succeed but the target path will be invalid)
...
TEST_F(FileSystemTest, testCreateFileSymbolicLink) {
    ...
    EXPECT_TRUE(
        bump::FileSystem::createDirectory("unittest/files/new_symlinks"));
    bump::String source = "unittest/files/output.txt";
    bump::String relative_source = "../output.txt";
    bump::String destination = "unittest/files/new_symlinks/output_copy.txt";
    EXPECT_TRUE(
        bump::FileSystem::createFileSymbolicLink(relative_source, destination));
    EXPECT_TRUE(bump::FileSystem::exists(destination));
    EXPECT_TRUE(bump::FileSystem::isSymbolicLink(destination));
    EXPECT_TRUE(bump::FileSystem::exists(source));
    EXPECT_TRUE(bump::FileSystem::isFile(source));
    ...
    // Test an incorrectly built relative path file symlink. It is really
    // important to note that all the tests will pass except for the last one.
    // We get the canonical path to the symlink's target to see if the symlink
    // is actually valid.
    ...
    // Try to create some symlink files with bad source paths (creation will
    // succeed but the target path will be invalid)
...
TEST_F(FileSystemTest, testRemoveSymbolicLink) {
    ...
    // Try to remove some files with a parent path symlink, fails since the
    // files themselves are not symlinks
    path = "unittest/symlink_directory/paper.doc";
    EXPECT_FALSE(bump::FileSystem::removeSymbolicLink(path));
    path = "unittest/symlink_directory/help.pdf";
    EXPECT_FALSE(bump::FileSystem::removeSymbolicLink(path));
...
TEST_F(FileSystemTest, testCopySymbolicLink) {
    ...
    // Copy a symlink file to a different directory. This will all go fine but
    // the target path will be wrong since it is a relative path. You can only
    // copy symlinks with absolute paths to avoid this issue.
    EXPECT_TRUE(
        bump::FileSystem::createDirectory("unittest/files/new_symlinks"));
    source = "unittest/symlink_files/output.txt";
    ...
    // Copy a symlink directory to a different directory. This will all go fine
    // but the target path will be wrong since it is a relative path. You can
    // only copy symlinks with absolute paths to avoid this issue.
    EXPECT_TRUE(bump::FileSystem::createDirectory(
        "unittest/files/new_symlink_directory"));
    source = "unittest/symlink_directory";
...
TEST_F(FileSystemTest, testRenameSymbolicLink) {
    ...
    // Rename a symlink file to a different directory. This will all go fine but
    // the target path will be wrong since it is a relative path. You can only
    // rename symlinks with absolute paths to avoid this issue.
    EXPECT_TRUE(
        bump::FileSystem::createDirectory("unittest/files/new_symlinks"));
    ...
    // Rename a symlink directory to a different directory. This will all go
    // fine but the target path will be wrong since it is a relative path. You
    // can only rename symlinks with absolute paths to avoid this issue.
    EXPECT_TRUE(bump::FileSystem::createDirectory(
        "unittest/files/new_symlink_directory"));
    source = "unittest/symlink_directory_copy";
    destination =
        "unittest/files/new_symlink_directory/symlink_directory_copy_copy";
    EXPECT_TRUE(bump::FileSystem::renameSymbolicLink(source, destination));
    EXPECT_TRUE(bump::FileSystem::exists(destination));
    EXPECT_TRUE(bump::FileSystem::isSymbolicLink(destination));
    EXPECT_FALSE(bump::FileSystem::exists(source));
