...
/**
 * Test fixture for the bump::Uuid class. There is no shared state or per-test
...
TEST_F(UuidTest, testEmptyConstructor) {
    ...
    EXPECT_STREQ("00000000-0000-0000-0000-000000000000",
...
TEST_F(UuidTest, testCopyConstructorBoost) {
    ...
    // Test the string versions
    EXPECT_STREQ(boost::uuids::to_string(uuid1).c_str(),
                 uuid2.toString().c_str());
}
...
TEST_F(UuidTest, testFromString) {
    ...
    EXPECT_STREQ("4605d211-2d5b-4ab4-8feb-d7c38e4e38c3",
                 uuid.toString().c_str());
    EXPECT_FALSE(uuid.isNull());
    ...
    EXPECT_THROW(bump::Uuid::fromString("this is NOT valid"),
                 bump::TypeCastError);
}
