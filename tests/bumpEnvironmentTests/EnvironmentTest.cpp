...
protected:
    ...
    void SetUp() {
        ...
        bump::Environment::setEnvironmentVariable(_environmentVariable,
                                                  _expectedValue);
    }
...
TEST_F(EnvironmentTest, testEnvironmentVariable) {
    ...
    bump::String actual_value =
        bump::Environment::environmentVariable(_environmentVariable);
...
TEST_F(EnvironmentTest, testSetEnvironmentVariable) {
    ...
    // Try to overwrite the default environment variable without forcing the
    // overwrite flag
    bump::Environment::setEnvironmentVariable(_environmentVariable, "Attempt 2",
                                              false);
    actual_value = bump::Environment::environmentVariable(_environmentVariable);
    EXPECT_STREQ(_expectedValue.c_str(), actual_value.c_str());

    // Try to overwrite the default environment variable again with forcing the
    // overwrite flag
    bump::Environment::setEnvironmentVariable(_environmentVariable, "Attempt 3",
                                              true);
    actual_value = bump::Environment::environmentVariable(_environmentVariable);
