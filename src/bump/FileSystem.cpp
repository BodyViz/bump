...
String join(const String& path1, const String& path2, const String& path3,
            const String& path4, const String& path5) {
    boost::filesystem::path joined_path(path1.c_str());
    joined_path /= path2.c_str();
    joined_path /= path3.c_str();
    joined_path /= path4.c_str();
...
String join(const String& path1, const String& path2, const String& path3,
            const String& path4, const String& path5, const String& path6) {
    boost::filesystem::path joined_path(path1.c_str());
    joined_path /= path2.c_str();
    joined_path /= path3.c_str();
    joined_path /= path4.c_str();
...
String join(const String& path1, const String& path2, const String& path3,
            const String& path4, const String& path5, const String& path6,
            const String& path7, const String& path8, const String& path9) {
    boost::filesystem::path joined_path(path1.c_str());
    joined_path /= path2.c_str();
    joined_path /= path3.c_str();
    joined_path /= path4.c_str();
...
bool createDirectory(const String& path) {
    try {
        return boost::filesystem::create_directory(
            boost::filesystem::path(path.c_str()));
    } catch (const boost::filesystem::filesystem_error& /*e*/) {
        return false;
    }
}
...
bool copyDirectoryAndContents(const String& source, const String& destination) {
    ...
    BOOST_FOREACH (const FileInfo& local_source_file, local_source_files) {
        ...
        if (local_source_file.isDirectory()) {
            ...
            // Copy the local source directory to the local destination
            // directory
            copied_successfully = copyDirectoryAndContents(
                local_source_file.absolutePath(), local_destination_directory);
            ...
            copied_successfully = copySymbolicLink(
                local_source_file.absolutePath(), local_destination_file);
        } else  // local_source_info.isFile()
        {
            ...
            copied_successfully = copyFile(local_source_file.absolutePath(),
...
StringList directoryList(const String& path) {
    ...
    if (!FileInfo(path).isDirectory()) {
        String msg =
            String("The following path is not a directory: %1").arg(path);
        throw FileSystemError(msg, BUMP_LOCATION);
    }
    ...
    BOOST_FOREACH (const boost::filesystem::path& item,
                   std::make_pair(iter, end_iter)) {
...
bool createDirectorySymbolicLink(const String& source,
                                 const String& destination) {
    try {
        // Convert the source path to the preferred OS
        boost::filesystem::path source_path(
            boost::filesystem::path(source.c_str()).make_preferred().string());
        boost::filesystem::path destination_path(destination.c_str());

        // Create the directory symlink
        boost::filesystem::create_directory_symlink(source_path,
                                                    destination_path);
...
bool setModifiedDate(const String& path, const std::time_t& date) {
    ...
    try {
        boost::filesystem::last_write_time(
            boost::filesystem::path(path.c_str()), date);
        return true;
    } catch (const boost::filesystem::filesystem_error& /*e*/) {
        return false;
    }
