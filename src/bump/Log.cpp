...
Log::Log()
    ...
    // Attempt to disable the entire log system based on the "BUMP_LOG_ENABLED"
    // environment variable
    String logEnabled =
        bump::Environment::environmentVariable(BUMP_LOG_ENABLED);
    logEnabled.toLowerCase();
    if (logEnabled == "no" || logEnabled == "false" || logEnabled == "nope" ||
        logEnabled == "disable") {
        _isEnabled = false;
    ...
    // Attempt to set the log level based on the "BUMP_LOG_LEVEL" environment
    // variable
    ...
    if (logLevel == "ALWAYS_LVL") {
        ...
        std::cout
            << "[bump] WARNING: Your BUMP_LOG_LEVEL environment variable: ["
            << logLevel
            << "] does not match any of the possible options: [ ALWAYS_LVL | "
    ...
    // Attempt to set the log file based on the "BUMP_LOG_FILE" environment
    // variable
    ...
    if (!logFile.empty()) {
        if (logFile == "stderr") {
            ...
            if (success) {
                std::cout << "[bump] Setting BUMP_LOG_FILE to " << logFile
                          << std::endl;
            } else {
                std::cout << "[bump] WARNING: Your BUMP_LOG_FILE environment "
                             "variable: ["
                          << logFile << "] could not be created or opened"
                          << std::endl;
            }
        }
    }
...
std::ostream& Log::logStream(const String& prefix) {
    ...
    // The generic stream to "buffer" output to for the output handler. This
    // allows us to avoid pointer dereferences until necessary.
...
String Log::convertTimeToString() {
    ...
    // Now create a string representation of the timestamp based on the
    // timestamp format
    if (_timestampFormat == DATE_TIME_TIMESTAMP) {
        return String("%1-%2-%3 %4:%5:%6:")
            .arg(year, month, day, hours, minutes, seconds);
    } else if (_timestampFormat == DATE_TIME_WITH_AM_PM_TIMESTAMP) {
        return String("%1-%2-%3 %4:%5:%6 %7:")
            .arg(year, month, day, hours, minutes, seconds, am_pm);
...
void bumpALWAYS(const bump::String& message) {
    bump::Log& log = bump::Log::instance();
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::ALWAYS_LVL)) {
        log.logStream() << message << std::endl;
    }
}
...
void bumpERROR(const bump::String& message) {
    bump::Log& log = bump::Log::instance();
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::ERROR_LVL)) {
        log.logStream() << message << std::endl;
    }
}
...
void bumpWARNING(const bump::String& message) {
    bump::Log& log = bump::Log::instance();
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::WARNING_LVL)) {
        log.logStream() << message << std::endl;
    }
}
...
void bumpINFO(const bump::String& message) {
    bump::Log& log = bump::Log::instance();
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::INFO_LVL)) {
        log.logStream() << message << std::endl;
    }
}
...
void bumpDEBUG(const bump::String& message) {
    bump::Log& log = bump::Log::instance();
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::DEBUG_LVL)) {
        log.logStream() << message << std::endl;
    }
}
...
void bumpALWAYS_F(const bump::String& message) {
    ...
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::ALWAYS_LVL)) {
        std::flush(log.logStream() << message);
    }
}
...
void bumpALWAYS_P(const bump::String& prefix, const bump::String& message) {
    ...
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::ALWAYS_LVL)) {
        log.logStream(prefix) << message << std::endl;
    }
}
...
void bumpALWAYS_PF(const bump::String& prefix, const bump::String& message) {
    ...
    boost::mutex::scoped_lock lock(log.convenienceFunctionMutex());
    if (log.isLogLevelEnabled(bump::Log::ALWAYS_LVL)) {
        std::flush(log.logStream(prefix) << message);
    }
}
