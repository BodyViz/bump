    ...
    for (unsigned int i = 1; i < beginningLine; i++) {
        ...
        if (input_file.eof()) {
            bumpERROR_P("FileReader: ",
                        "The line requested is larger than the number of lines "
                        "in the file");
            return file_contents;
        }
    }
...
StringList fileContents(const String& fileName, int beginningLine,
                        int numLines) {
    if (beginningLine < 1) {
...
StringList header(const String& fileName, int numLines) {
    if (numLines < 1) {
        bumpINFO_P("FileReader: ",
                   "The numLines can not be less than 1 for a header");
        StringList empty_string;
        return empty_string;
    }
...
StringList footer(const String& fileName, int numLines) {
