...
String::String(int number) { *this = boost::lexical_cast<std::string>(number); }
...
String::String(float number, int precision) {
    ...
    if (precision < -1) {
        throw InvalidArgumentError("Cannot handle a precision less than -1",
                                   BUMP_LOCATION);
    }
    ...
    if (precision == -1) {
        result << std::setprecision(std::numeric_limits<float>::digits10 + 1)
               << number;
    } else {
...
String String::arg(const String& argument) const {
    ...
    if (markers.empty()) {
        throw StringSearchError("Could not find any markers (i.e. %1 - %99",
                                BUMP_LOCATION);
    }

    // Now we need to find the lowest marker so we can replace it. We also need
    // to store each marker's position in a multimap to be able to replace only
    // the exact portion of the string. For example, we need to ensure we don't
    // replace %1 and %10 at the same time. To make sure we only replace the %1
    ...
    // Iterate through the marker position map and replace all the lowest
    // markers. We need to also keep track of the difference in length between
    // our match text and the replacement argument text as "skipped_space". Our
    // replacement positions need to then be adjusted by the skipped space each
    // time to map properly.
    int skipped_space = 0;
    std::multimap<int, int>::iterator iter =
        marker_position_map.find(lowest_marker_value);
    for (iter = marker_position_map.find(lowest_marker_value);
         iter != marker_position_map.end(); ++iter) {
        // Stop once we're finished with the lowest markers
        if (iter->first != lowest_marker_value) {
            break;
        }

        // Actually replace the marker with the argument using skipped_space to
        // make sure we're in the right spot regardless of how many iterations
        // we've gone through.
        int start_position = iter->second;
        replaced.replace(start_position + skipped_space, lowest_marker.length(),
                         argument);
...
const char& String::at(int position) const {
    try {
        ...
        throw OutOfRangeError("Position is outside string bounds",
                              BUMP_LOCATION);
    }
}
...
bool String::compare(const String& otherString,
...
bool String::compare(const char* otherString,
                     CaseSensitivity caseSensitivity) const {
    return compare(String(otherString), caseSensitivity);
}

bool String::contains(const String& containString,
                      CaseSensitivity caseSensitivity) const {
    // Make some copies to handle case sensitivity
    String this_copy = *this;
...
bool String::contains(const char* containString,
                      CaseSensitivity caseSensitivity) const {
    return contains(String(containString), caseSensitivity);
}

int String::count(const String& containString,
                  CaseSensitivity caseSensitivity) const {
    // Make some copies to handle case sensitivity
    String this_copy = *this;
    String contain_string_copy = containString;
...
int String::count(const char* containString,
                  CaseSensitivity caseSensitivity) const {
...
bool String::endsWith(const String& endString,
                      CaseSensitivity caseSensitivity) const {
    if (caseSensitivity == NotCaseSensitive) {
...
bool String::endsWith(const char* endString,
                      CaseSensitivity caseSensitivity) const {
...
String& String::fill(const String& character, int size) {
    ...
    if (character.length() != 1) {
        throw InvalidArgumentError(
            "Passed invalid character...must be length of one", BUMP_LOCATION);
    }

    // Make sure the size is not less than -1
    if (size < -1) {
        throw InvalidArgumentError(
            "Passed invalid size, must not be less than -1", BUMP_LOCATION);
    }
...
int String::indexOf(const String& indexString, int startPosition,
                    CaseSensitivity caseSensitivity) const {
...
int String::indexOf(const char* indexString, int startPosition,
                    CaseSensitivity caseSensitivity) const {
...
String& String::insert(const String& insertString, int position) {
    ...
    String start;
    String end;
    if (position > 0) start = section(0, position);
    if (length() > position) end = section(position, this->length());
...
bool String::isEmpty() const { return std::string::empty(); }
...
int String::lastIndexOf(String indexString, int startPosition,
                        CaseSensitivity caseSensitivity) const {
    // Make sure the index string passed in is not empty
    if (indexString.empty()) {
        throw InvalidArgumentError("Passed empty index string", BUMP_LOCATION);
...
int String::lastIndexOf(const char* indexString, int startPosition,
                        CaseSensitivity caseSensitivity) const {
...
String String::left(int n) const {
    ...
    if (n > length() || n < 1) {
        throw OutOfRangeError("The n parameter is outside string bounds",
                              BUMP_LOCATION);
    }
...
String& String::remove(int position, int n) {
    ...
    if (n < 0) {
        throw InvalidArgumentError("The n parameter cannot be negative",
                                   BUMP_LOCATION);
    }
...
String& String::remove(const String& removeString,
...
String& String::remove(const char* removeString,
                       CaseSensitivity caseSensitivity) {
...
String& String::replace(int position, int n, const String& replaceString) {
    ...
    remove(position, n);
    if (!replaceString.isEmpty()) insert(replaceString, position);
    return *this;
}
...
String& String::replace(const String& before, const String& after,
    ...
    if (before.isEmpty()) {
        throw InvalidArgumentError("Passed an empty before string",
                                   BUMP_LOCATION);
    }
...
String String::section(int startPosition, int length) const {
    ...
    if (length < 1) {
        throw InvalidArgumentError("Length must be at least one",
                                   BUMP_LOCATION);
    }
...
StringList String::split(const String& separator) const {
    ...
    boost::algorithm::split(split_strings, *this, boost::is_any_of(separator),
                            boost::token_compress_on);

    // Convert the vector to a StringList
    StringList converted_strings;
    converted_strings.insert(converted_strings.end(), split_strings.begin(),
                             split_strings.end());

    return converted_strings;
}

bool String::startsWith(const String& startString,
                        CaseSensitivity caseSensitivity) const {
    if (caseSensitivity == NotCaseSensitive) {
...
bool String::startsWith(const char* startString,
                        CaseSensitivity caseSensitivity) const {
...
std::string String::toStdString() const { return *this; }
