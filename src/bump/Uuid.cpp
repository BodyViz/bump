...
Uuid Uuid::fromString(const String& uuidString) {
    try {
        ...
        throw TypeCastError(
            String("Could not convert %1 to uuid").arg(uuidString),
            BUMP_LOCATION);
    }
}

bool Uuid::isNull() const { return boost::uuids::uuid::is_nil(); }

String Uuid::toString() const { return boost::uuids::to_string(*this); }
