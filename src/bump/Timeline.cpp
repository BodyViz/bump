...
Timeline::Timeline(double duration, const Direction& direction,
...
void Timeline::stop() { _state = NOT_RUNNING; }
...
void Timeline::update() {
    ...
    if (_state == PAUSED) {
        std::cout << "Timeline update() is returning because it is PAUSED."
                  << std::endl;
        return;
        ...
        std::cout
            << "Timeline update() is returning because it is NOT_RUNNING.\n"
            << std::endl;
        return;
    }
    ...
    if (_runTime >= _duration) _state = FINISHED;
...
Timeline::Direction Timeline::direction() { return _direction; }

void Timeline::setCurveShape(const CurveShape& curveShape) {
    _curveShape = curveShape;
}

Timeline::CurveShape Timeline::curveShape() { return _curveShape; }

void Timeline::setDuration(double duration) { _duration = duration; }

double Timeline::duration() { return _duration; }

Timeline::State Timeline::state() { return _state; }

double Timeline::stepValue() { return _stepValue; }

double Timeline::stepIncrement() { return _stepIncrement; }

void Timeline::calculateAcceleration() {
    if (_curveShape == EASE_IN_AND_OUT_CURVE) {
        _acceleration =
            (_endOutput - _startOutput) / ((_duration / 2) * (_duration / 2));
    } else {
        _acceleration =
            2.0 * (_endOutput - _startOutput) / (_duration * _duration);
    }
}
...
void Timeline::generateStep() {
    ...
    if (_curveShape == EASE_IN_CURVE) {
        if (_direction == FORWARDS) {
            newStepValue =
                ((_acceleration * _runTime * _runTime) / 2.0) + _startOutput;
            _stepIncrement = newStepValue - _stepValue;
            _stepValue = newStepValue;
        } else {
            newStepValue =
                _endOutput - ((_acceleration * _runTime * _runTime) / 2.0);
            _stepIncrement = newStepValue - _stepValue;
            _stepValue = newStepValue;
        }
    } else if (_curveShape == EASE_OUT_CURVE)  // EASE_OUT_CURVE
    {
        if (_direction == FORWARDS) {
            newStepValue =
                ((-_acceleration * inverseDuration * inverseDuration) / 2.0) +
                _endOutput;
            _stepIncrement = newStepValue - _stepValue;
            _stepValue = newStepValue;
        } else {
    ...
    } else if (_curveShape == EASE_IN_AND_OUT_CURVE)  // EASE_IN_AND_OUT_CURVE
    {
        ...
        if (_runTime < _halfDuration) {
            if (_direction == FORWARDS) {
                newStepValue = ((_acceleration * _runTime * _runTime) / 2.0) +
                               _startOutput;
                _stepIncrement = newStepValue - _stepValue;
                _stepValue = newStepValue;
            } else {
            ...
            if (_direction == FORWARDS) {
                newStepValue =
                    ((-_acceleration * inverseDuration * inverseDuration) /
                     2.0) +
                    _endOutput;
                _stepIncrement = newStepValue - _stepValue;
                _stepValue = newStepValue;
    ...
    } else  // LINEAR_CURVE
    {
        if (_direction == FORWARDS) {
            newStepValue =
                (_runTime / _duration) * (_endOutput - _startOutput) +
                _startOutput;
            _stepIncrement = newStepValue - _stepValue;
            _stepValue = newStepValue;
        } else {
            newStepValue = ((_duration - _runTime) / _duration) *
                               (_endOutput - _startOutput) +
                           _startOutput;
            _stepIncrement = newStepValue - _stepValue;
            _stepValue = newStepValue;
        }
